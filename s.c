...
A1(str0,asrt(xtC);x=room(x,1);xc[xn]=0;x)A1(str,!xtt?ea1(str,x):xtc?enl(x):xts?mR(symstr(gs(x))):kst(x))S A2 ks2;A1(kst,ks2(x,aC(0)))
...
A2(ks2,P(xtl,sl(gl(x),y))P(xtiI,apc(ks2(gL(x),y),'i'))P(xt==tdx,sd(gd(x),y))P(xtT&&xn==1,ksr(fir(x),apc(y,',')))
 ...
 P(xtcC,x=enla(x);I b=0,n=xn,m=n+2;F(n,C c=xci,e=esc(c);$((c<32||126<c)&&!e,b=1;B)m+=!!e)A z=aC(b?2+2*n:m);C*s=zc;
  ...
  h(z);xr;y)
 P(xtT,$(xtX,hc('('))
       $(xn,C c=xtX?';':xtS?0:32;F(xn,$(i&&c,hc(c))y=ks2(get(x,i),y)))E$(xtL||xtI,hs("!0"))E$(xtD,hs("0#0n"))E$(xtS,hs("0#`"))
       $(xtX,hc(')'))xr;y)
