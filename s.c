...
S A2(ksl,ksp(x,y,xtX||xtC?xn==1:xtL||xtD||xtS?xn<=1:xtaA||     xtq||xtr||xtu||xtv||xtw))
S A2(kso,ksp(x,y,xtX||xtC?xn==1:xtL||xtD||xtS?xn<=1:xtaA||xtp||xtq||               xtw))
A2(kst,P(xtl,strL(gl(x),y))P(xt==tdx,strD(gd(x),y))P(xtT&&xn==1,ksr(fir(x),apc(y,',')))
 ...
 P(xtcC,x=enla(x);I b=0,n=xn,m=n+2;F(n,C c=xci,e=esc(c);$((c<32||126<c)&&!e,b=1;B)m+=!!e)A z=aC(b?2+2*n:m);C*s=zc;
  ...
  h(z);xr;y)
 P(xtT,$(xtX,hc('('))$(xn,C c=xtX?';':xtS?0:32;F(xn,$(i&&c,hc(c))y=kst(get(x,i),y)))E$(xtL,hs("!0"))E$(xtD,hs("0#0n"))E$(xtS,hs("0#`"))$(xtX,hc(')'))xr;y)
