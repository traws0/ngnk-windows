...
S C*su(C*s,UL x)_(x<TX?sX(s,x):S0(s4(s,x/TX),x%TX))C*sl(C*s,Lv)_(Y(v<0,P(v==NL,Mh(s,"0N"))v=-v;*s++='-')su(s,v))
...
S C*sd(C*s,L d)_(UL x=d,m=x<<12>>12;I e=x<<1>>53;P(m&&e==2047,Mh(s,"0n"))Y(x>>63,*s++='-')P(e==2047,Mh(s,"0w"))P(!m&&!e,Mh(s,"0.0"))
 ...
 P(in(t,16),Ik=l<t?l:t;Mm(s,s+1,k);Ms(s+l,'0',max(0,t-l));s+=t;*s++='.';s+=l-k;Y(t>=l,*s++='0')s)
...
A1(str,Xzd(I d=xtd;Ay=aC(24);AN((d?sd:sl)(yC,gl(x))-yC,y))Xs(Lv=gi(x);aCz(syp(&v)))Xc(enl(x))XF(kst(x))K("$:'",x))

S A pre(Cc,Ax)_(cat(ac(c),x))A sur(Cc,Ax,Cd)_(apc(pre(c,x),d))S A1(par,sur('(',x,')'))
S A1(raz,rdc(cv(','),A(x),1))S I esc(Cc)_(i(6,P(c=="\0\t\n\r\"\\"[i],i))-1)S A1(ek,eac1(kst,x))
...
A1(kst,P(c3(tA,xt,tS)&&xn==1,pre(',',ksr(fir(x))))XA(par(jc(';',ek(x))))XcC(kss(enla(x)))
 XZD(xn?jc(32,eac1(str,x)):x(aCz(xtD?"0#0n":"!0")))XS(xn?raz(ek(x)):x(aCz("0#`")))XM(pre('+',kst(flp(x))))
...
A1(hex,Xc(hex(enl(x)))XC(Ay=aC(2*xn);C*s=yC;x2(i(xn,*s++=h[xc>>4];*s++=h[xc&15])y))et1(x))
