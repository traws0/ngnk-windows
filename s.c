...
S I cin(Cc,OC*s)_(s[si(s,c)])S A pre(Cc,Ax)_(cat(ac(c),x))S A sur(Cc,Ax,Cd)_(apc(pre(c,x),d))
S A raz(Ax)_(rdc(cv(','),A(x),1))S I esc(Cc)_(i(6,P(c=="\0\t\n\r\"\\"[i],i))-1)S A ek(Ax)_(ea1(kst,x))
...
A1(kst,P(c3(tA,xt,tS)&&xn==1,pre(',',ksr(fir(x))))XA(sur('(',join(ac(';'),ek(x)),')'))Xc(kss(enl(x)))XC(kss(x))
 ...
 Xp(x=val(x);P(xn-3?0:!cin(TS[At(xx)],"vr")?0:xy-PLH&&xz==PLH,Ay=kv(&x);x(cat(ksl(y),kst(x))))
    x=ek(x);Nm=An(xx);x=apc(join(ac(';'),x),']');Q(xr==1);xC[m]='[';x)
 Xq(raz(ek(val(x))))Xr(x=val(x);Ay=kv(&x);cat(ksw(x),str(y)))Xw(atnv(tC,1+xv/3,&"':/:\\:"[xv%3*2]))
 P(xtu||xtv,P(x==PLH,aC0)Ay=atnv(tC,1,&vc[xv]);i(xtu+(19<xv),y=apc(y,':'))y)aCz("???"))
A1(out,P(x==au0,x)dex(v0c(ai(1),xtA&&1<xn?sur('(',join(aCz("\n "),ek(xR)),')'):kst(xR)),x))
