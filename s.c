...
A1(kst,P(c3(tA,xt,tS)&&xn==1,pre(',',ksr(fir(x))))XA(par(joc(';',ek(x))))XcC(kss(enla(x)))
 XZD(xn?joc(32,eac1(str,x)):x(aCz(xtD?"0#0n":"!0")))XS(xn?raz(ek(x)):x(aCz("0#`")))XM(pre('+',kst(flp(x))))
 ...
 Xp(x=val(x);P(xn-3?0:!cin(TS[_t(xx)],"vr")?0:xy-PLH&&xz==PLH,Ay=mR(xy);cat(ksl(y),kst(fir(x))))
    x=ek(x);Nm=_n(xx);x=apc(joc(';',x),']');Q(xr==1);xC[m]='[';x)
...
A1(out,P(x==au0,x)dex(v0c(ai(1),xtA&&1<xn?par(joC("\n ",2,ek(xR))):kst(xR)),x))
