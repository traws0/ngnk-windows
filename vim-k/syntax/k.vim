...
sy match  k_e       /\i\+\|\S/
sy match  k_s       /\(`\(\I\i*\>\|:[A-Za-z0-9.:/]*\)\=\)\+/         nextgroup=@k_nvw                         |hi link k_s       constant
...
sy match  k_es      /.*/                                             nextgroup=@k_nvw contained               |hi link k_es      k_string
sy match  k_string  /"\(\\.\|[^"\n]\)*"/                             nextgroup=@k_nvw contains=k_q            |hi link k_string  string
sy match  k_q       /\\./                                                             contained               |hi link k_q       specialchar
sy match  k_u       /[:+\-*%!&|<>=~,^#_$?@.]:\=/                     nextgroup=k_w                            |hi link k_u       function
sy match  k_v       /\s*[+\-*%!&|<>=~,^#_$?@.]:\=/                   nextgroup=k_w    contained               |hi link k_v       type
sy match  k_i       /\(\.\=[a-zA-Z\u0400-\u047f0-9]*\)\+/            nextgroup=@k_nvw                         |hi link k_i       variable
sy match  k_x       /\<[oxyz]\>/                                     nextgroup=@k_nvw                         |hi link k_x       special
sy match  k_n       /\v\s*([a-zA-Z0-9)\]}]@<!\-)=\d+([NWnw]|\.\d+)=/ nextgroup=@k_nvw                         |hi link k_n       number
sy match  k_string  /\<0x\(\x\x\)*\>/                                nextgroup=@k_nvw
sy match  k_u       /\d::\=/                                         nextgroup=k_w
sy match  k_v       /\s*\d::\=/                                      nextgroup=k_w    contained
sy match  k_g       /:/                                                                                       |hi link k_g       statement
sy region k_ar      matchgroup=k_a start=/(/                 end=/)/ nextgroup=@k_nvw contains=@k_k,k_aj,k_ae |hi link k_a       nontext
sy region k_br      matchgroup=k_b start=/\[/                end=/]/ nextgroup=@k_nvw contains=@k_k,k_bj,k_be |hi link k_b       k_a
sy region k_cr      matchgroup=k_c start=/{\(\[[^\]]*\]\)\=/ end=/}/ nextgroup=@k_nvw contains=@k_k,k_cj,k_ce |hi link k_c       special
...
sy cluster k_nvw contains=k_n,k_v,k_w
