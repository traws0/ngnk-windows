...
I fpA(A*p,Ay/*1*/)_(Ax=*p;In=xn,i=fAA(x,y);P(i<0,_q(*p,y);n)mr(y);i)
...
A2(fnd,
 ...
 YA(ear(cv('?'),A(x,y),2))
 P(yt-xt,rsh(len(x(y)),mR(cn[tl])))
 Au=aL(yn);TY(fC)*f=CH(Zt[xt],&fC,fH,fI,fL);i(un,ul=NINL(f(xC,xn,yC+i*ZT[xt])))x(y(u)))
