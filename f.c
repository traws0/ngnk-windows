...
S A rnd(Ln,L m)_(Y(n==NL,Ed(m<0)n=-m)Ax=aL(absL(n));rndl(xL,xn);Y(m,Y(m<=1ll<<32,i(xn,xl=(UI)xl*(UL)m>>32))E(i(xn,xl=(UL)xl%m)))
...
A2(fnd,
 ...
 Yt(P(!xtA&&fun(y),xr;yr;al(NL))
     fir(fnd(x,enl(y))))
 XA(Au=aL(yn);i(un,Az=get(y,i);ul=NINL(fAA(x,z));zr)xr;yr;u)
 YA(ear(cv('?'),A(x,y),2))
 P(yt-xt,xr;rsh(len(y),mR(cn[tl])))
 Au=aL(yn);TY(fC)*f=CH(Zt[xt],&fC,fH,fI,fL);i(un,ul=NINL(f(xC,xn,yC+i*ZT[xt])))xr;yr;u)
