...
S A rnd(Ln,L m)_(Y(n==NL,Ed(m<0)n=-m)Ax=aL(absL(n));rndl(xL,xn);Y(m,Y(m<=1ll<<32,F(xn,xl=(UI)xl*(UL)m>>32))E(F(xn,xl=(UL)xl%m)))
...
A2(fnd,
 ...
 Yt(P(!xtA&&fun(y),xr;yr;al(NL))
     fir(fnd(x,enl(y))))
 XA(Au=aL(yn);F(un,Az=get(y,i);ul=NINL(fAA(x,z));zr)xr;yr;u)
 YA(ear(cv('?'),A(x,y),2))
