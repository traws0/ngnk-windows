...
#define f_(T)  NO I  f##T(OV*p,I n,OV*q)_(O T*a=p,v=*(T*)q;i(n,P(a[i]==v,i))-1)
#define fp_(T) NO I fp##T(A*p,T v)_(Ax=*p;In=xn,i=f##T(xC,n,&v);P(i<0,*p=apv(x,&v);n)i)
f_(B)f_(H)f_(I)f_(L) fp_(B)fp_(H)fp_(I)fp_(L) //fB fH fI fL fpB fpH fpI fpL
...
A2(fnd,Xs(SF(x,y,"``j`hex`aes",des,jsr,unh,aes1))
 ...
 Ym(fir(N(fnd(x,enl(y)))))
 XZ(Yz(Lv=gl(y);TY(fB)*f=CH(Zt[xt],&fB,fH,fI,fL);x(az(NNL(f(xC,xn,&v)))))
    YZ(N(sup(&x,&y));Au=aL(yn);TY(fB)*f=CH(Zt[xt],&fB,fH,fI,fL);x(y(iul(NNL(f(xC,xn,yC+i*ZT[xt]))))))
    K("{(#y)#0N}",x,y))
 P(xtS&&ytsS,fnd(AT(tI,x),AT(ytt?ti:tI,y)))
 P(xtC&&ytcC,fnd(AT(tB,x),AT(ytt?tb:tB,y)))
 P(xtD&&ytdD,fnd(AT(tL,x),AT(ytt?tl:tL,y)))
 Yt(fir(N(fnd(x,enl(y)))))
 Au=aL(yN);x(y(iul(Az=get(y,i);dex(z,NNL(fA(x,z)))))))
