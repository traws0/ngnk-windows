...
#define f_(T)  NO I  f##T(OV*p,I n,L v0)_(O T*a=p,v=(T)v0;i(n,P(a[i]==v,i))-1)
...
A2(fnd,Xs(SF(x,y,"``j`hex`aes",des,jsr,unh,aes1))
 ...
 XZ(Yz(Lv=gl(y);P(xt<tZ(v),x(y(mR(cn[tl]))))TY(fB)*f=CH(Zt[xt],&fB,fH,fI,fL);x(az(NNL(f(xC,xn,v)))))
    YZ(N(sup(&x,&y));Au=aL(yn);x(m2(y,xtB?iul(NNL(fB(xC,xn,yb))):
                                      xtH?iul(NNL(fH(xC,xn,yh))):
                                      xtI?iul(NNL(fI(xC,xn,yi))):
                                          iul(NNL(fL(xC,xn,yl))))))
    K("{(#y)#0N}",x,y))
