...
A1(til,Xz(Ln=gl(x);P(n<0,add(az(n),til(az(-n))))Ez(n>1<<30)x=atn(tZ(n-1),n);CH(xt-tB,&iB,iH,iI,iL)(xC,n);x)
...
S I c(Ax,Ay/*00*/)_(P(mtc_(x,y),0)Iv=cT(x)-cT(y);P(v,v)P(xtc||xtz,cL(gl_(x),gl_(y)))
 Xd(Dv=gd_(x),w=gd_(y);v<w?-1:v>w?1:0)Xs(Lv=gi(x),w=gi(y);strcmp(syp(&v),syp(&w)))
