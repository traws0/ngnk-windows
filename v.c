...
A1(rev,P(xtt,x)P(xta,AT(ta,N(ea1(rev,AT(tX,x)))))P(xtA,A y=gkv(&x);aA(x,Nx(ea1(rev,y))))
 x=mut(x);L j=xn,m=(xn+1)/2;I w=tZ(xt);P(w==1,F(m,--j;SW(xci,xcj))x)
                                       P(w==4,F(m,--j;SW(xii,xij))x)
                                       P(w==8,F(m,--j;SW(xli,xlj))x)UR;0)
A1(typ,UC t=xt;xr;as(syC(tsym[t])))A1(len,dex(x,al(len_(x))))L len_(A x/*0*/)_(xtT?xn:xta?len_(xy):!xtA?1:An(xy)?len_(*(A*)dat(xy)):1)
