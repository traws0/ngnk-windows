...
A1(gC,XC(x)x=N(gI(x));Xt(ac(xv))Ay=aC(xn);x2(cIC(xC,yC,yn);y))
A1(gB,XB(x)x=N(gI(x));Xt(x)     Ay=aB(xn);x2(cIB(xC,yC,yn);y))
A1(gH,XH(x)x=N(gI(x));Xt(x)     Ay=aH(xn);x2(cIH(xC,yC,yn);y))
A1(gI,XI(x)Xzc(ai(gl(x)))XdD(gI(N(gL(x))))XZC(Ay=aI(xn);x2(CH(xt-tB,&cBI,cHI,0,cLI,0,cCI)(xC,yC,yn);y))et1(x))
A1(gL,XlL(x)Xzc(al(gl(x)))Xt(gt(x,gL))XD(Ay=aL(xn);x2(cDL(xC,yC,yn);y))x=N(gI(x));Ay=aL(xn);x2(cIL(xC,yC,yn);y))
A1(gD,XdD(x)Xzc(ad(gl(x)))Xt(gt(x,gD))x=N(gL(x));Ay=aD(xn);x2(cLD(xC,yC,yn);y))
...
S C ot(Ax)_(xtz?tl:xtZ?tL:xt)S L od(Dv)_(Li=*(L*)&v;i^(UL)(i>>63)>>1)S I qL(Li,Lj)_(i<j?-1:i>j)I qD(D u,Dv)_(qL(od(u),od(v)))
S I cA(Ax,Ay/*00*/)_(P(mtc_(x,y),0)P(xtd&&ytd,qD(*xD,*yD))P(xtzc&&ytd,qD(gl_(x),*yD))P(xtd&&ytzc,qD(*xD,gl_(y)))
 Iv=ot(x)-ot(y);P(v,v)Xzc(qL(gl_(x),gl_(y)))Xd(*xD<*yD?-1:*xD>*yD?1:0)Xs(strcmp(syp(&x),syp(&y)))
