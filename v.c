...
A1(flp,XM(Ay=kv(&x);am(x,y))Xm(Ay=kv(&x);P(!ytA||!yn,er2(x,y))Ln=cfm(yA,yn);i(yn,Az=ya;P(ztt,y=mut(y);ya=rshN(n,z)))aM(x,y))
 Xt(enl(enl(x)))P(!xtA||!xn,enl(x))Ln=cfm(xA,xn);P(n==-1,enl(x))P(n<0,el1(x))Ay=aA(n);i(n,Az=oA;j(xn,zq(get(xA[j],i)))ya=z)x(y))
