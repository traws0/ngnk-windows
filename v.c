...
NI A tili(I n)_(A x=aI(n);F(n,xii=i)x)
A1(til,P(xti,I n=gi (x);n<0&&n-_0Ni?add(x,N(tili(-n))):tili(n    ))
       P(xtl,L n=gl_(x);n<0&&n-_0Nl?add(x,N(till(-n))):till(gl(x)))
...
A1(typ,UC t=xt;xr;as(syc(tsym[t])))A1(len,dex(x,al(len_(x))))L len_(A x/*0*/)_(xtT?xn:xta?len_(xy):!xtA?1:An(xy)?len_(*(A*)data(xy)):1)
...
A1(gS,P(xtsS,x)P(xtC,x=str0(x);dex(x,as(sym(xc))))P(xtc,as(syc(gc(x))))et(x))
