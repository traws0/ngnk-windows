...
A1(flp,XM(AT(tm,mut(x)))Xm(Ay=xy;Er(!ytA||!yn,x)i(yn,Az=ya;En(!ztT,x))Lm=An(yx);i(yn,Az=ya;El(zn-m,x))AT(tM,mut(x)))
 Xt(enl(enl(x)))P(!xtA||!xn,enl(x))K("{(,/n#'x)(n*!#x)+/:!n:|/#'x}",x))
A1(til,Xz(Ln=gl(x);P(n<0,add(az(n),til(az(-n))))Ez(n>1<<30)Au=atn(tT(xt),n);CH(ut-tB,&iB,iH,iI,iL)(uC,n);u)
 XZ(K("{x((*a)#&#)'1_a:|*\\|x,1}",x))Xm(fir(AT(tA,mut(x))))Xo(val(x))Xs(til(N(enl(x))))XS(til(N(val(x))))Et(!xtA||xn,x)x)
...
A1(rev,XT(x=mut(x);CH(Zt[xt],&rB,rH,rI,rL,rL)(xC,xC+ZT[xt]*(xn-1));x)
 Xm(AT(tm,N(ea1(rev,AT(tA,mut(x))))))XM(Ay=kv(&x);aM(x,Nx(ea1(rev,y))))x)
A1(typ,x(as(TS[xt])))
...
A1(gB,XbB(x)Xz(ab(gl(x)))Xt(gt(x,gB))x=N(gI(x));Au=aB(xn);m2(x,cIB(xC,uC,un);u))
A1(gH,XhH(x)Xz(ah(gl(x)))Xt(gt(x,gH))x=N(gI(x));Au=aH(xn);m2(x,cIH(xC,uC,un);u))
...
A1(pI,XmMA(ea1(pI,x))Xc(pI(enl(x)))XC(x=str0(x);C*s=xC;P(!*s,x(mR(cn[tl])))Lv=pl(&s);x(*s?mR(cn[tl]):az(v)))et1(x))
...
A1(asc,XA(Ez(xn-(I)xn,x)A*a=xA,u=til(ai(xn));In=xn,i=n/2;W(i-->0,sft(a,uI,i,n-1))i=n-1;W(i>0,SWP(ui,*uI)sft(a,uI,0,--i))x(u))
