...
A1(unq,XT(xn<2?x:K("{x@&@[&#x;i;:;]@[;0;:;a=a:*a]@a:~$[`A=@x;~;=]':x@i:<x}",x))Xz(que(x,ad(1)))et1(x))
A2(fil,YmMA(eac2(x,y,cv('^')))YF(y==au0?x:x(y))Yt(fir(fil(x,enl(y))))K("{@[y;&^y;:;x]}",x,y))
A2(exc,Xt(fil(x,y))En(xtmM||ytmM,x,y)y=enla(y);P(!xn||!yn,y(x))K("{x@&~(x~\\:y),!0}/",x,y))
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(xtCZ);Ay=atn(t+1,n);x2(CH(t-tB,&cBH,cHI,cIL,cLD,0,cCH)(xC+ZT[t]*i,yC,j-i);y))
...
A1(gI,XiI(x)Xz(ai(gl(x)))XdD(gI(N(gL(x))))Xcz(ai(gl(x)))XCZ(Ay=aI(xn);x2(CH(xt-tB,&cBI,cHI,0,cLI,0,cCI)(xC,yC,yn);y))et1(x))
...
TY(ct)ct={[tA]=blw,gB,gH,gI,gL,gD,gC,gS};
A2(cst,P(xtS||ytmMA,eac2(x,y,cv('$')))P(xtz&&ytcC,K("{y@(!x)+(x<0)*#y}",x,enla(y)))
 Xs(Ss("b","h","i","l","d","c","","s","I")Sf(x,y,s,gB,gH,gI,gL,gD,gC,gS,gS,pI))en2(x,y))
...
L tru(Ax/*1*/)_(Lv=c3(ti,xt,ts)?gl_(x):xtF?!xtu||xv:xn;x(0);v)
