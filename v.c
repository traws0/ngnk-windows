...
A1(gI,XiI(x)XdD(gI(N(gL(x))))Xl(gt(x,gI))XL(Au=aI(xn);m2(x,cLI(xC,uC,un);u))
 XC(Au=aI(xn);m2(x,cCI(xC,uC,un);u))Xc(ai(gc(x)))
 XB(Au=aI(xn);m2(x,cBI(xC,uC,un);u))Xb(ai(gb(x)))
 XH(Au=aI(xn);m2(x,cHI(xC,uC,un);u))Xh(ai(gh(x)))et1(x))
