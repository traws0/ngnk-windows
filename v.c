...
A1(til,P(xti,In=gi(x);I m=n<0;n*=1-2*m;Au=aI(n);tilI(ui,n);m?sub(u,ai(n)):u)
       P(xtl,Ln=gl(x);I m=n<0;n*=1-2*m;Au=aL(n);tilL(ul,n);m?sub(u,al(n)):u)
...
A1(gI,P(xtiI,x)P(xtdD,gI(N(gL(x))))P(xtl,gt(x,gI))P(xtL,Au=aI(xn);m2(x,cLI(xl,ui,un);u))P(xtc,ai(gc(x)))et(!xtC,x)Au=aI(xn);m2(x,cCI(xc,ui,un);u))
...
A2(cst,P(ytmMA,eac(cv('$'),A(x,y),2))P(xtil&&ytC,ap1(y,Ny(til(x))))en(!xts,x,y)
 Ii=gI(K("``s`c`i`l`j`d`f?",x));ed(i<0,y)(A1*[]){gS,gS,gC,gI,gL,gL,gD,gD}[i](y))
A1(sqz,P(!xtA||!xn,x)Ay=xx;Ct=yt;P(!sim(y)&&t-tm,x)Ln=xn;F(n,Ay=xai;P(t-yt,x))P(t==tm,Az=yx;F(n,Ay=xai;P(!mtc_(yx,z),x))aM(zR,flp(ea1(val,x))))
 t=t_lst(t);Au=atn(t,n);I w=ZT[t];asrt(!reft(t));P(w==1,m2(x,cLC(xl,uc,n);u))P(w==4,m2(x,cLI(xl,ui,n);u))asrt(w==8);F(n,Ay=xai;uli=*yl);xr;u)
...
S I cmp_(Ax,Ay/*x0y0*/)_(P(x==y,0)I d=xt-yt;P(d,d)P(xti,gi(x)-gi(y))P(xtc,gc(x)-gc(y))P(xtl,cmpL(gl_(x),gl_(y)))P(xtd,D u=gd_(x),v=gd_(y);u<v?-1:u>v?1:0)
