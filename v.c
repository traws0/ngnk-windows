...
L tru(A x/*1*/)_(mr2(x,sim(x)?gl_(x):fun(x)?!xtu||Av(x):xn))A2(dex,xr;y)A1(fir,xtt?x:dex(x,getr(x,0)))A1(las,xtt?x:dex(x,getr(x,xn-1)))
...
A1(whr,P(xtA||xtX,et(x))P(xta,A y=gkv(&x);idx(x,Nx(whr(y))))
 x=enla(x);x=N(gL(x));L n=0;F(xn,P(xli<0,ed(x))n+=xli)A u=aL(n);n=0;mr2(x,F(xn,Fj(xli,ul[n++]=i))u))
...
A1(typ,mr2(x,as(sym(atnv(tC,1,&tsym[xt])))))A1(len,dex(x,al(len_(x))))L len_(A x/*0*/)_(xtT?xn:xta?len_(xy):!xtA?1:An(xy)?len_(*A(xy)):1)
