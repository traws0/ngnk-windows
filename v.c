...
A1(typ,UC t=xt;xr;as(symc(tsym[t])))A1(len,dex(x,al(len_(x))))L len_(A x/*0*/)_(xtT?xn:xta?len_(xy):!xtA?1:An(xy)?len_(*(A*)data(xy)):1)
...
A1(gS,xtsS?x:xtC?as(sym(xc)):xtc?as(symc(gc(x))):et(x))
