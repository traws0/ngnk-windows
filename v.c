...
A2(cut,Ym(K("{((!y)^x)#y}",x,y))YT(x=Ny(gL(x));K("{n:#y;y$[`l=@x;$[n<x|-x;!0;x<0;!n+0^x;x+!n-x];x+!'1_-':x,#y]}",x,y))
...
A1(grp,Xt(Et(!xtil)K("{(2#x)#1i,`i$&x}",x))Xm(Au=grp(kv(&x));Q(Ar(u)==1);uy=idx(x,uy);u)Ay=atn(xt,0),u=a0();C w=Zt[xt];
 i(xn,Lj=!w?fpC(&y,xc):w==2?fpI(&y,xi):w==3?fpL(&y,xl):fpA(&y,mR(xa));Y(j==un,uq(aL(0)))uA[j]=apv(uA[j],&i))xr;am(y,u))
