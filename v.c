...
A1(til,P(xtX,xn?ed(x):x)P(xto,val(x))P(xta,fir(AT(tX,x)))P(xtl,L n=gl(x);A u=aL(llabs(n));$(n<0,F(-n,uli=i+n))E(F(n,uli=i))u)P(!xtL,et(x))
...
A2(cut,
 ...
 xr;yr;u)
S A1(gC,P(xtcC,x)P(xtl,ac(gl(x)))P(xtL,A u=aC(xn);F(un,uci=(C)xli)xr;u)et(x))
A1(gL,P(xtlL,x)P(xtc,al(gc(x)))P(xtC,A u=aL(xn);F(un,uli=(UC)xci)xr;u)et(x))
A1(gD,P(xtdD,x)x=N(gL(x));P(xtt,ad(gl(x)))asrt(ZL==Z(D));xr;A u=aD(xn);F(un,udi=xli)u)
