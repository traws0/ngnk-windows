...
A1(til,Xz(Ln=gl(x);P(n<0,n-NL?add(az(n),til(az(-n))):ed0())Ez(n>1<<30)x=atn(tZ(n-1),n);CH(xt-tB,&iB,iH,iI,iL)(xC,n);x)
 XZ(K("{x((*a)#&#)'1_a:|*\\|x,1}",x))Xm(x(_R(xx)))Xo(val(x))Xs(til(N(enl(x))))XS(til(N(val(x))))Et(!xtA||xn,x)x)
A1(whr,XA(K("{$[`A~@x;(,&#'*'x),,'/x@\\:!0|/#'x:o'x;,&x]}",x))Xm(Ay=kv(&x);idx(x,Nx(whr(y))))Et(xtM,x)
 x=N(gL(enla(x)));Ln=0;i(xn,Ed(xl<0,x)n+=xl)Ct=tZ(xn-1);Ez(t>tI,x)Au=atn(t,n);x2(n=0;CH(t-tB,&wB,wH,wI)(xL,xn,uC);u))
...
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(c3(tC,t,tI));Au=atn(t+1,n);x2(CH(t-tC,&cCH,cBH,cHI,cIL,cLD)(xC+ZT[t]*i,uC,j-i);u))
S A gt(Ax,A1*f)_(fir(N(f(enl(x)))))
A1(gC,XcC(x)Xz(ac(gl(x)))Xt(gt(x,gC))x=N(gI(x));Au=aC(xn);x2(cIC(xC,uC,un);u))
A1(gB,XB( x)Xz(gI(x))    Xt(gt(x,gB))x=N(gI(x));Au=aB(xn);x2(cIB(xC,uC,un);u))
A1(gH,XH( x)Xz(gI(x))    Xt(gt(x,gH))x=N(gI(x));Au=aH(xn);x2(cIH(xC,uC,un);u))
...
A1(sqz,P(!xtA||!xn,x)Au=oA;i(xn,uq(_R(xa)))x(u))
...
A1(asc,XCZ(In=xn;Ez(n-xn,x)x=N(K("{x-&/x}",x));Ay=aC(n),z=aI(n),u=aI(n);x2(y2(ascZ(xC,yC,zI,uI,n,ZT[xt])==zI?u(z):z(u))))
...
A1(grp,Xz(K("{x=/:x:!x}",x))Xt(et1(x))Xm(Au=grp(kv(&x));uy=idx(x,uy);u)K("{x[*'g]!g@:<g:(&(~x~':x i),!0)_i:<x}",x))
