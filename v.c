...
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(xtZC);Ay=an(t+1,n);xe(T(&cBH,cHI,cIL,cLD,0,cCH)[t-tB](xV+TZ[t]*i,yV,j-i);y))
SN A cZ(Ax,Ct)_(P(xt==t,x)x=N(cI(x));Xt(t==tC?ac(xv):x)Ay=an(t,xn);xe(T(&cIB,cIH,0,0,0,cIC)[t-tB](xV,yV,yn);y))
