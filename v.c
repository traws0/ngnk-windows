...
A1(flp,P(xtmM,Y(xtm,Ay=xy;er(!ytA||!yn,x)F(yn,Az=ya;en(!ztT,x))L m=An(*yA);F(yn,Az=ya;el(zn-m,x)))AT(xt^tm^tM,x))P(xtt,enl(enl(x)))
...
A1(whr,et(xtM||xtA,x)P(xtm,Ay=gkv(&x);idx(x,Nx(whr(y))))x=N(gL(enla(x)));Ln=0;F(xn,ed(xl<0,x)n+=xl)Au=aL(n);m2(x,n=0;F(xn,Fj(xl,uL[n++]=i))u))
...
A1(sqz,P(!xtA||!xn,x)Ay=xx;Ct=yt;P(!sim(y)&&t-tm,x)Ln=xn;F(n,Ay=xa;P(t-yt,x))
 ...
 t=tT(t);Au=atn(t,n);P(Zt[t]<3,m2(x,(V(*[])(OV*RE,V*RE,UL)){cLC,cLH,cLI}[Zt[t]](xL,uC,n);u))F(n,Ay=xa;ul=*yL);xr;u)
A1(blw,P(xtA,x)P(xtt,a1(x))et(xtm,x)Ln=len_(x);Au=aA(n);F(n,ua=get(x,i))xr;u)
...
A1(asc,P(xtA,ez(xn>WI,x)A*a=xA,u=til(ai(xn));In=xn,i=n/2;W(i-->0,sft(a,uI,i,n-1))i=n-1;W(i>0,SWP(ui,*uI)sft(a,uI,0,--i))xr;u)
