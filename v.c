...
A1(whr,Xt(XF(et1(x))whr(enl(x)))XA(K("{$[`A~@x;(,&#'*'x),,'/x@\\:!0|/#'x:o'x;,&x]}",x))
 ...
 Ay=atn(t,n);xe(I(t==tB,B*a=yV;i(xn,j(xl,*a++=i)))
...
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(xtZC);Ay=atn(t+1,n);xe(T(&cBH,cHI,cIL,cLD,0,cCH)[t-tB](xV+TZ[t]*i,yV,j-i);y))
SN A cZ(Ax,Ct)_(P(xt==t,x)x=N(cI(x));Xt(t==tC?ac(xv):x)Ay=atn(t,xn);xe(T(&cIB,cIH,0,0,0,cIC)[t-tB](xV,yV,yn);y))
A1(cC,cZ(x,tC))S A1(cB,cZ(x,tB))A1(cH,cZ(x,tH))
A1(cI,XI(x)Xzc(ai(gl(x)))XdD(cI(N(cL(x))))XZC(Ay=aI(xn);Ii=xt-tB;xe(T(&cBI,cHI,0,cLI,0,cCI)[i](xV,yV,yn);y))et1(x))
...
A1(sqz,P(!xtA||!xn,x)Nn=xn;Ay=xx;Ct=yt;Ym(y=oA;i(n,yq(_R(xa)))x(y))P(!c3(ti,t,ts),x)
 ...
 E(i(n,P(t-_t(xa),x)))
 y=atn(TT[t],n);Iv=Tz[yt];I(v<3,T(&cLB,cLH,cLI)[v](xV,yV,n))E(i(n,yl=gl_(xa)))x(y))
