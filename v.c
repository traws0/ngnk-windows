...
A1(til,P(xth||xtil,Ln=gl_(x);Im=n<0;n*=1-2*m;Ez(n>1<<30,x)Au=atn(tT(xt),n);CH(xt-th,&tilH,tilI,tilL)(uC,n);(m?add:dex)(x,u))
 P(xtH||xtIL,K("{x((*a)#&#)'1_a:|*\\|x,1}",x))Xm(fir(AT(tA,x)))Xo(val(x))Xs(Iv=gs(x);e0(syp(&v)))Et(!xtA||xn,x)x)
A1(whr,Et(xtM||xtA,x)Xm(Ay=kv(&x);idx(x,Nx(whr(y))))x=N(gL(enla(x)));Ln=0;i(xn,Ed(xl<0,x)n+=xl)
 Au=aL(n);m2(x,n=0;i(xn,j(xl,uL[n++]=i))u))
...
A1(sqz,P(!xtA||!xn,x)Ay=xx;Ct=yt;P(!sim(y)&&t-tm,x)Ln=xn;i(n,Ay=xa;P(t-yt,x))
 ...
 t=tT(t);Au=atn(t,n);P(Zt[t]<3,m2(x,CH(Zt[t],&cLC,cLH,cLI)(xL,uC,n);u))i(n,Ay=xa;ul=*yL);xr;u)
A1(blw,XA(x)Xt(a1(x))Et(xtm,x)Ln=len_(x);Au=aA(n);i(n,ua=get(x,i))xr;u)
L tru(Ax/*1*/)_(Lv=sim(x)?gl_(x):fun(x)?!xtu||xv:xn;xr;v)A1(fir,xtt?x:dex(x,getr(x,0)))A1(las,xtt?x:dex(x,getr(x,xn-1)))
I mtc_(Ax,Ay/*x0y0*/)_(P(x==y,1)P(xt-yt||pkd(x)||xn-yn||(xtr&&xv-yv),0)P(!ref(x),i(xn*ZT[xt],P(xc-yc,0))1)i(xn,P(!mtc_(xa,ya),0))1)
...
S I c(Ax,Ay/*x0y0*/)_(P(x==y,0)Iv=xt-yt;P(v,v)P(xtc||xth||xti,gi(x)-gi(y))Xl(cij(gl_(x),gl_(y)))
 ...
 Y(!pkd(x),Iv=cij(xn,yn);XT(i(min(xn,yn),Az=get(x,i),u=get(y,i);I d=c(z,u);zr;ur;P(d,d))v))cij(x,y))
...
S A1(asci,Ez(xn>WI,x)x=N(K("{x-&/x}",x));I w=ZT[xt];UI n=xn;Ay=aC(n),z=aI(n),u=aI(n);C*a=dat(y);UI c[257];tilI(uI,n);
 m2(x,m2(y,j(w,ms(c,0,Z c);i(n,a[i]=xC[w*ui+j])i(n,c[a[i]+1]++)Y(c[1+*a]-n,i(255,c[i+1]+=c[i])i(n,zI[c[a[i]]++]=ui)SWP(z,u)))zr;u)))
...
A1(grp,Xt(Et(!xtil)K("{(2#x)#1i,`i$&x}",x))Xm(Au=grp(kv(&x));Q(Ar(u)==1);uy=idx(x,uy);u)Ay=atn(xt,0),u=aA0;C w=Zt[xt];
 i(xn,Lj=!w?fpC(&y,xc):w==2?fpI(&y,xi):w==3?fpL(&y,xl):fpA(&y,mR(xa));Y(j==un,uq(aL0))uA[j]=apv(uA[j],&i))xr;am(y,u))
