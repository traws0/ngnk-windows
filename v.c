...
A1(flp,XM(Ay=kv(&x);am(x,y))Xm(Ay=kv(&x);P(!ytA||!yn,er2(x,y))Ln=cfm(yA,yn);i(yn,Az=ya;P(ztt,y=mut(y);ya=rshN(n,z)))aM(x,y))
 ...
 Ct=_t(*xA);I(t<tM,i(xn,Ay=xa;B(yt-t,t=0)))E(t=0)Ay=aA(n);I w=Tz[t];
 P(!t,i(n,Az=aA(xn);j(xn,zA[j]=get(xA[j],i))ya=sqz(z))x(y))
 i(n,Az=atn(t,xn);I(!w,j(xn,Ay=xA[j];zB[j]=yb))EI(w==1,j(xn,Ay=xA[j];zH[j]=yh))EI(w==2,j(xn,Ay=xA[j];zI[j]=yi))
                  E(j(xn,Ay=xA[j];zL[j]=yl)I(w==4,;z=sqz(mRa(z))))ya=z)x(y))
