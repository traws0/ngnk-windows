...
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(xtZC);Ay=atn(t+1,n);xe(CH(t-tB,&cBH,cHI,cIL,cLD,0,cCH)(xV+TZ[t]*i,yV,j-i);y))
SN A cZ(Ax,Ct)_(P(xt==t,x)x=N(cI(x));Xt(t==tC?ac(xv):x)Ay=atn(t,xn);xe(CH(t-tB,&cIB,cIH,0,0,0,cIC)(xV,yV,yn);y))
A1(cC,cZ(x,tC))S A1(cB,cZ(x,tB))A1(cH,cZ(x,tH))
A1(cI,XI(x)Xzc(ai(gl(x)))XdD(cI(N(cL(x))))XZC(Ay=aI(xn);Ii=xt-tB;xe(CH(i,&cBI,cHI,0,cLI,0,cCI)(xV,yV,yn);y))et1(x))
...
A2(cst,P(xtS||ytmMA,eac2f(x,y,cst))
 ...
 Xs(Iv=xv;P(v-(C)v,ed1(y))CH(si("idcsI",v+'s'*!v),&csti,cD,cC,cS,pI,ed1)(y))
 et2(x,y))
