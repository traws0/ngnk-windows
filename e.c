...
CD A err0(O C*m)_($((UL)m<128,m=eM[(L)m])en=min(strlen(m),Z(em)-1);mc(em,m,en);em[en]=0;0)
CD A err1(O C*m,A x)_(xr;err0(m))CD A err2(O C*m,A x,A y)_(yr;err1(m,x))CD A err3(O C*m,A x,A y,A z)_(zr;err2(m,x,y))
CD A errn(O C*m,I n,O A*a)_(mrn(n,a);err0(m))CD S I ecl()_(en=0;$(es,mr(es);es=0)eo=0)CD I eso(A x/*1*/,I o)_($(!es,es=x;eo=o)0)
CD I epr()_(I h=64;C s0[3*h+Z(em)+6],*s=s0;*s++='\'';mc(s,em,en);s+=en;*s++=10;
