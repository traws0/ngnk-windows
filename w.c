...
AX(rdc,P(xtilIL,n>1?ern(n,a):K("{z+x*y}/[0;;]",xR,*a))P(xtcC,join(x,a,n))P(Ak(x)<2,cvg(x,a,n))
 A y=n>1?n--,*a++:0,z=*a;L m=cfm(a,n);P(m==-1,y?app(x,a-1,n+1):*a)P(m<0,Y(y,yr)eln(n,a))P(!m&&!y,A u=id_(x,z);mrn(n,a);u)
 Y(xtv&&Av(x)<11&&(!y||ytt)&&n==1&&(ztC||ztIL||ztD),Y(ztC,z=gL(z);P(!z,Y(y,yr);0))Y(y&&ytdD||ztdD,Y(y,y=gD(y))z=gD(z))
  P(ztI,I*p=zi,a=!y?m--,*p++:gi(y);mr2(z,((V(*)(V*,V*,L))arf[zt-tI][Av(x)][3])(&a,p,m);0);ai(a))
  P(ztL,L*p=zl,a=!y?m--,*p++:gl(y);mr2(z,((V(*)(V*,V*,L))arf[zt-tI][Av(x)][3])(&a,p,m);0);al(a))
  P(ztD,D*p=zd,a=!y?m--,*p++:gd(y);mr2(z,((V(*)(V*,V*,L))arf[zt-tI][Av(x)][3])(&a,p,m);0);ad(a)))
 L i=!y;Y(i,y=get(z,0))A b[8];W(i<m,*b=y;Fj(n,b[j+1]=get(a[j],i))y=app(x,b,n+1);Y(!y,B)i++)mrn(n,a);y)
S AX(cvi,A y,z,u;Y(n==1,y=0;z=*a)EY(n==2,y=*a;z=a[1])E(R ern(n,a))
...
AX(scn,P(xtilIL,enc(x,a,n))P(xtcC,spl(x,a,n))P(Ak(x)<2,cvi(x,a,n))
 P(n==1,A z=*a;P(ztt||!zn,z)P(zta,z=mut(z);zy=scn(x,&zy,1);z)P(x==cv(','),scn(x,A_(a0(),z),2))
  P(xtv&&Av(x)<11&&(ztIL||ztD||ztC),Y(ztC,z=N(gL(z)))
   I w=tZ(zt);A u=ax(z);mc(uc,zc,w);((V(*)(V*,V*,V*,L))arf[zt-tI][Av(x)][4])(zc,zc+w,uc+w,zn-1);u)
...
AX(eap,P(n==1&&xtilIL,eac(x,a,n))P(n>2,ern(n,a))A z=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);P(!ztT,et(y,z))P(!zn,yr;z)
 Y(xtv&&Av(x)<11,
  P(yti&&(ztI||ztC) ,I a=gi(gI(y));z=N(gI(z));A u=ax(z);((V(*)(V*,V*,V*,L))arf[zt-tI][Av(x)][5])(&a,zc,uc,zn);u)
  P(ytil&&(ztL||ztC),L a=gl(gL(y));z=N(gL(z));A u=ax(z);((V(*)(V*,V*,V*,L))arf[zt-tI][Av(x)][5])(&a,zc,uc,zn);u)
  P(ytd||ztD        ,D a=gd(gD(y));z=N(gD(z));A u=ax(z);((V(*)(V*,V*,V*,L))arf[zt-tI][Av(x)][5])(&a,zc,uc,zn);u))
