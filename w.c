...
AX(eac,Ab8;Ct[8];Lm=-1;
 ...
 Au=0;Y(!m,Y(x==LEN,u=oL)EY(n==2&&xtv&&xv<11,u=a[!_N(a[1])];uR)E(u=oA))
...
AX(rdc,Ay=n>1?n--,*a++:0,z=*a;Lm=cfm(a,n);P(m==-1,y?app(x,a-1,n+1):z)P(m<0,Y(y,y(0))eln(n,a))P(!m&&!y,x=id_(x,z);mrn(n,a);x)
 ...
 Li=!y;Y(i,y=get(z,0))Ab8;W(i<m,*b=y;j(n,b[j+1]=get(a[j],i))y=app(x,b,n+1);B(!y)i++)mrn(n-1,a+1);z(y))
AX(scn,
...
AX(w3,eap(x,a,n))
AX(w4,n-2?ern(n,a):ear(*a,a[1],x))A3(ear,/*110*/z=prj(z,A(x,PLH),2);z(eac(z,&y,1)))
AX(w5,n-2?ern(n,a):eal(*a,a[1],x))A3(eal,/*110*/z=prj(z,A(PLH,y),2);z(eac(z,&x,1)))
