...
S AX(enc,P(n>1,ern(n,a))P(xtiI,x=gL(xR);Au=enc(x,a,n);xr;u)P(!xtlL,etn(n,a))
 ...
 P(!xn,yr;aA0)Au=aA(xn);y=mut(y);Li=xn;W(--i>0,UL m=xl;m+=!m;Az=ua=aL(yn);i(yn,UL v=yl,w=yl=v/m;zl=v-w*m))
...
A ea1(A1 f,Ax)_(Xt(f(x))Xm(x=mut(x);xy=ea1(f,xy);P(!xy,xy=au0;xr;0)x)I b=xtA&&Ar(x)==1;Ln=len_(x);P(!n,x)Au=aA0;
...
AX(scn,P(xtilIL,enc(x,a,n))P(xtcC,spl(x,a,n))P(xk<2,cvi(x,a,n))
 ...
 Ay=*a++;n--;Lm=cfm(a,n);P(m==-2,yr;eln(n,a))Au=aA0,b[8];
...
AX(eap,P(n==1&&xtilIL,eac(x,a,n))P(n>2,ern(n,a))Az=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);Et(!ztT,y,z)P(!zn,yr;z)
 ...
 Au=aA0;i(zn,y=app(x,A(get(z,i),y),2);P(!y,zr;ur;0)uq(y);y=get(z,i))yr;zr;u)
