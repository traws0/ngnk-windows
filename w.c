...
AX(rdc,P(xtilIL,n>1?ern(n,a):K("{z+x*y}/[0;;]",xR,*a))P(xtcC,join(x,a,n))P(Ak(x)<=1,cvg(x,a,n))
 ...
 $(ari(x)&&(!y||ytt)&&n==1,$(ztC,z=gL(z);P(!z,$(y,yr);0))$(y&&ytdD||ztdD,$(y,y=gD(y))z=gD(z))
  P(ztI,I*p=zi,a=!y?m--,*p++:gi(y);mr2(z,arI(Av(x),m,I b=p[i];a=)ai(a)))
  P(ztL,L*p=zl,a=!y?m--,*p++:gl(y);mr2(z,arL(Av(x),m,L b=p[i];a=)al(a)))
  P(ztD,D*p=zd,a=!y?m--,*p++:gd(y);mr2(z,arD(Av(x),m,D b=p[i];a=)ad(a))))
