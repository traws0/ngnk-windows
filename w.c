...
A jc(Cc,   Ax)_(Ln=jn(1,x);Et(n<0,x)Ay=aC(n);C*p=yC;i(xn,Y(i,*p++=c        )Az=xa;Y(ztc,*p++=gi(z))E(Mc(p,zC,zn);p+=zn))x(y))
A jC(Qs,Nm,Ax)_(Ln=jn(m,x);Et(n<0,x)Ay=aC(n);C*p=yC;i(xn,Y(i,Mc(p,s,m);p+=m)Az=xa;Y(ztc,*p++=gi(z))E(Mc(p,zC,zn);p+=zn))x(y))
...
S AX(eacm,Az=oS,b[8];i(n,Ay=a[i];Y(ytm,z=cat(z,_R(yx))))z=unq(z);
 i(n,Ay=b[i]=a[i];Y(ytm,Au=kv(&y);uq(id_(x,u));b[i]=ap1(u,fil(ai(_n(y)),fnd(y,zR)))))Au=eac(x,b,n);u?am(z,u):z(u))
...
A eac1(A1 f,Ax)_(Xt(f(x))Xm(x=mut(x);xy=eac1(f,xy);P(!xy,xy=au0;x(0))x)I b=xtA&&xr==1;Nn=xN;P(!n,x)
...
AX(rdc,Y(n==1,XzZ(dec(xR,*a))Xc(jc(gi(x),*a))XC(jC(xC,xn,*a)))P(xk<2,cvg(x,a,n))
 ...
 Y(n==1&&xtv&&xv<8&&(!y||ytt)&&ztCZD,Y(ztC,z=gH(z))Y(y&&(ytd||ztD),y=gD(y);z=gD(z))Ct=zt;
  int(*f)(V*,V*,N)=arr[t-tB][xv];P(f,Lv=gl(y?y:get(z,0));z2(f(&v,zC+ZT[t]*!y,m-!y));t==tD?atnv(td,1,&v):az(v)))
 Li=!y;Y(i,y=get(z,0))A b[8];W(i<m,*b=y;j(n,b[j+1]=get(a[j],i))y=app(x,b,n+1);B(!y)i++)mrn(n,a);y)
AX(scn,Y(n==1,XzZ(enc(xR,*a))Xc(splc(gi(x),*a))XC(splC(xC,xn,*a)))P(xk<2,cvi(x,a,n))
 P(n==1,Az=a[n-1];P(ztt||!zn,z)P(ztm,Ay=kv(&z);y=Nz(scn(x,&y,1));am(z,y))P(x==cv(','),scn(x,A(oA,z),2))
  Y(xtv&&xv<8&&ztCZD/*todo*/&&(xv-1&&xv-2&&xv-3||ztL||ztD),Y(ztC,z=N(gL(z)))
   int(*f)(V*,V*,V*,N)=ars[zt-tB][xv];P(f,I w=ZT[zt];Au=ax(z);Mc(uC,zC,w);f(zC,zC+w,uC+w,zn-1);u))
...
AX(eap,XzZ(eac(x,a,n))P(n>2,ern(n,a))Az=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);Et(!ztT,y,z)P(!zn,y(z))
 Y(xtv&&xv<11&&ytCZ&&ztCZ/*todo*/&&(xv-1&&xv-2&&xv-3||ztL),Y(ytz,y=az(gl(y)))N(sup(&y,&z));Lv=gl(y);
  int(*f)(V*,V*,V*,N)=arp[zt-tB][xv];P(f,Au=ax(z);f(&v,zC,uC,zn);u))
