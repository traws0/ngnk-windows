...
AX(rdc,XzZ(AXA2(x,a,n,dec))XcC(AXA2(x,a,n,join))P(xk<2,cvg(x,a,n))
 ...
 P(n==1&&xtv&&xv<8&&(!y||ytt)&&c3(tC,zt,tD),Y(ztC,z=gH(z))Y(y&&(ytd||ztD),y=gD(y);z=gD(z))
  Lv=gl(y?y:get(z,0));m2(z,((int(*)(V*,V*,L))arr[zt-tH+1][xv])(&v,zC+ZT[zt]*!y,m-!y));ztD?atnv(td,1,&v):az(v))
...
AX(scn,XzZ(AXA2(x,a,n,enc))XcC(AXA2(x,a,n,spl))P(xk<2,cvi(x,a,n))
 {Au=a[n-1];P(utt,A b[8];mc(b,a,(n-1)*Z(V*));b[n-1]=enl(u);scn(x,b,n))}
 P(n==1,Az=*a;P(ztt||!zn,z)P(ztm,Ay=kv(&z);y=Nz(scn(x,&y,1));am(z,y))P(x==cv(','),scn(x,A(aA0,z),2))
  P(xtv&&xv<8&&c3(tC,zt,tD),Y(ztC,z=N(gL(z)))
   I w=ZT[zt];Au=ax(z);mc(uC,zC,w);((int(*)(V*,V*,V*,L))ars[zt-tH+1][xv])(zC,zC+w,uC+w,zn-1);u)
  Ay=get(z,0),u=enl(yR);Li=0,n=zn;W(++i<n,y=N(app(x,A(y,get(z,i)),2));uq(yR))y(z(u)))
 Y(n==2,Au=a[1];P(utm,u=mut(u);uy=scn(x,A(*a,uy),2);u)P(!un,mr(*a),u))
...
AX(eap,XzZ(eac(x,a,n))P(n>2,ern(n,a))Az=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);Et(!ztT,y,z)P(!zn,y(z))
 Y(xtv&&xv<11,TD int(*T)(V*,V*,V*,L);
  P(yti&&(ztI||ztC),I a=gi(gI(y));z=N(gI(z));Au=ax(z);((T)arp[zt-tH+1][xv])(&a,zC,uC,zn);u)
  P(ytz&&(ztL||ztC),L a=gl(gL(y));z=N(gL(z));Au=ax(z);((T)arp[zt-tH+1][xv])(&a,zC,uC,zn);u)
  P(ytd||ztD       ,D a=gd(gD(y));z=N(gD(z));Au=ax(z);((T)arp[zt-tH+1][xv])(&a,zC,uC,zn);u))
