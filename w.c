...
AX(rdc,XzZ(AXA2(x,a,n,dec))XcC(AXA2(x,a,n,join))P(xk<2,cvg(x,a,n))
 ...
 P(n==1&&xtv&&xv<8&&(!y||ytt)&&c3(tC,zt,tD),Y(ztC,z=gH(z))Y(y&&(ytd||ztD),y=gD(y);z=gD(z))Ct=zt;
  Lv=gl(y?y:get(z,0));m2(z,((int(*)(V*,V*,L))arr[t-tB][xv])(&v,zC+ZT[t]*!y,m-!y));t==tD?atnv(td,1,&v):az(v))
...
AX(scn,XzZ(AXA2(x,a,n,enc))XcC(AXA2(x,a,n,spl))P(xk<2,cvi(x,a,n))
 P(n==1,Az=a[n-1];P(ztt||!zn,z)P(ztm,Ay=kv(&z);y=Nz(scn(x,&y,1));am(z,y))P(x==cv(','),scn(x,A(aA0,z),2))
  P(xtv&&xv<8&&c3(tC,zt,tD),Y(ztC,z=N(gL(z)))
   I w=ZT[zt];Au=ax(z);mc(uC,zC,w);((int(*)(V*,V*,V*,L))ars[zt-tB][xv])(zC,zC+w,uC+w,zn-1);u)
...
AX(eap,XzZ(eac(x,a,n))P(n>2,ern(n,a))Az=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);Et(!ztT,y,z)P(!zn,y(z))
 Y(xtv&&xv<11,TD int(*T)(V*,V*,V*,L);
  P(yti&&(ztI||ztC),I a=gi(gI(y));z=N(gI(z));Au=ax(z);((T)arp[zt-tB][xv])(&a,zC,uC,zn);u)
  P(ytz&&(ztL||ztC),L a=gl(gL(y));z=N(gL(z));Au=ax(z);((T)arp[zt-tB][xv])(&a,zC,uC,zn);u)
  P(ytd||ztD       ,D a=gd(gD(y));z=N(gD(z));Au=ax(z);((T)arp[zt-tB][xv])(&a,zC,uC,zn);u))
