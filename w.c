...
S A3(eac2,/*110*/P(ztv,eac2f(x,y,v2[zv]))Xt(Yt(app2(x,y,z))ear2(x,y,z))Yt(eal2(x,y,z))Xm(eacm(z,A(x,y),2))Ym(eacm(z,A(x,y),2))
 Nm=xN;P(m-yN,el2(x,y))P(!m,y(ztv&&zv<11?x:x(oA)))Au=0;Ct=t234(x),s=t234(y);
 i(m,A v=app2(t==2?xa:get(x,i),s==2?ya:get(y,i),z);B(!v,I(u,u=u(0))I(t==2,mrn(m-i-1,xA+i+1))I(s==2,mrn(m-i-1,yA+i+1)))
  I(!u,u=atn0(v,m))uq(v))
