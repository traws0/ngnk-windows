...
AX(rdc,XzZ(AXA2(x,a,n,dec))XcC(AXA2(x,a,n,join))P(xk<2,cvg(x,a,n))
 ...
 Y(n==1&&xtv&&xv<8&&(!y||ytt)&&c3(tC,zt,tD),Y(zt<tH,z=gH(z))Y(y,N(sup(&y,&z)))int(*f)(V*,V*,L)=arw[zt-tH][xv][0];
...
AX(scn,XzZ(AXA2(x,a,n,enc))XcC(AXA2(x,a,n,spl))P(xk<2,cvi(x,a,n))
 P(n==1,Az=*a;P(ztt||!zn,z)P(ztm,z=mut(z);zy=scn(x,&zy,1);z)P(x==cv(','),scn(x,A(aA0,z),2))
  P(xtv&&xv<8&&c3(tC,zt,tD),Y(ztC,z=N(gL(z)))
   I w=ZT[zt];Au=ax(z);mc(uC,zC,w);((int(*)(V*,V*,V*,L))arw[zt-tH][xv][1])(zC,zC+w,uC+w,zn-1);u)
...
AX(eap,P(n==1&&xtzZ,eac(x,a,n))P(n>2,ern(n,a))Az=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);Et(!ztT,y,z)P(!zn,y(z))
 Y(xtv&&xv<11,TD int(*T)(V*,V*,V*,L);
  P(yti&&(ztI||ztC),I a=gi(gI(y));z=N(gI(z));Au=ax(z);((T)arw[zt-tH][xv][2])(&a,zC,uC,zn);u)
  P(ytz&&(ztL||ztC),L a=gl(gL(y));z=N(gL(z));Au=ax(z);((T)arw[zt-tH][xv][2])(&a,zC,uC,zn);u)
  P(ytd||ztD       ,D a=gd(gD(y));z=N(gD(z));Au=ax(z);((T)arw[zt-tH][xv][2])(&a,zC,uC,zn);u))
