...
AX(rdc,P(xtilIL,n>1?ern(n,a):K("{z+x*y}/[0;;]",xR,*a))P(xtcC,join(x,a,n))P(Ak(x)<=1,cvg(x,a,n))
 ...
 Y(xtv&&Av(x)<8&&(!y||ytt)&&n==1,Y(ztC,z=gL(z);P(!z,Y(y,yr);0))Y(y&&ytdD||ztdD,Y(y,y=gD(y))z=gD(z))
  P(ztI,I*p=zi,a=!y?m--,*p++:gi(y);mr2(z,a=((int      (*)(I,I*,L))arf[0][Av(x)][3])(a,p,m));ai(a))
  P(ztL,L*p=zl,a=!y?m--,*p++:gl(y);mr2(z,a=((long long(*)(L,L*,L))arf[1][Av(x)][3])(a,p,m));al(a))
  P(ztD,D*p=zd,a=!y?m--,*p++:gd(y);mr2(z,a=((double   (*)(D,D*,L))arf[2][Av(x)][3])(a,p,m));ad(a)))
...
AX(scn,P(xtilIL,enc(x,a,n))P(xtcC,spl(x,a,n))P(Ak(x)<=1,cvi(x,a,n))
 P(n==1,A z=*a;P(ztt||!zn,z)P(zta,z=mut(z);zy=scn(x,&zy,1);z)P(x==cv(','),scn(x,A_(a0(),z),2))
  Y(xtv&&Av(x)<8,Y(ztC,z=N(gL(z)))
   P(ztI,A u=ax(z);I a=*ui=*zi;((void(*)(I,I*,I*,L))arf[0][Av(x)][4])(a,zi+1,ui+1,zn-1);u)
   P(ztL,A u=ax(z);L a=*ul=*zl;((void(*)(L,L*,L*,L))arf[1][Av(x)][4])(a,zl+1,ul+1,zn-1);u)
   P(ztD,A u=ax(z);D a=*ud=*zd;((void(*)(D,D*,D*,L))arf[2][Av(x)][4])(a,zd+1,ud+1,zn-1);u))
...
AX(eap,P(n==1&&xtilIL,eac(x,a,n))P(n>2,ern(n,a))A z=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);P(!ztT,et(y,z))P(!zn,yr;z)
 Y(xtv&&Av(x)<11,
  P(yti&&(ztI||ztC) ,I a=gi(gI(y));z=N(gI(z));A u=ax(z);((void(*)(I,I*,I*,L))arf[0][Av(x)][5])(a,zi,ui,zn);u)
  P(ytil&&(ztL||ztC),L a=gl(gL(y));z=N(gL(z));A u=ax(z);((void(*)(L,L*,L*,L))arf[1][Av(x)][5])(a,zl,ul,zn);u)
  P(ytd||ztD        ,D a=gd(gD(y));z=N(gD(z));A u=ax(z);((void(*)(D,D*,D*,L))arf[2][Av(x)][5])(a,zd,ud,zn);u))
