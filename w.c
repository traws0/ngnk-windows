...
NO L jn(Ax)_(XA(Nn=0;i(xn,Ay=xa;P(!ytcC,-1)n+=yN)n)-1)
A joc(Cc,Ax)_(Et(!xtA,x)Nn=xn,l=jn(x);Et(l<0,x)l+=n-!!n;
 Au=aC(l);C*p=uC;i(n,Y(i,*p++=c        )Az=xa;Y(ztc,*p++=gi(z))E(mc(p,zC,zn);p+=zn))x(u))
A joC(Qs,Nm,Ax)_(Et(!xtA,x)Nn=xn,l=jn(x);Et(l<0,x)l+=m*(n-!!n);
 Au=aC(l);C*p=uC;i(n,Y(i,mc(p,s,m);p+=m)Az=xa;Y(ztc,*p++=gi(z))E(mc(p,zC,zn);p+=zn))x(u))
A2(join,Xc(joc(gi(x),y))XC(x(joC(xC,xn,y)))en2(x,y))
...
A eac1(A1 f,Ax)_(Xt(f(x))Xm(x=mut(x);xy=eac1(f,xy);P(!xy,xy=au0;x(0))x)I b=xtA&&xr==1;Nn=xN;P(!n,x)Au=oA;
...
AX(scn,XzZ(AX2(enc))XcC(AX2(spl))P(xk<2,cvi(x,a,n))
 P(n==1,Az=a[n-1];P(ztt||!zn,z)P(ztm,Ay=kv(&z);y=Nz(scn(x,&y,1));am(z,y))P(x==cv(','),scn(x,A(oA,z),2))
  P(xtv&&xv<8&&c3(tC,zt,tD)/*todo*/&&(xv-1&&xv-2&&xv-3||ztL||ztD),Y(ztC,z=N(gL(z)))
