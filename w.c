...
AX(rdc,P(xtilIL,n>1?ern(n,a):K("{z+x*y}/[0;;]",xR,*a))P(xtcC,join(x,a,n))P(Ak(x)<=1,cvg(x,a,n))
 ...
 Y(ari(x)&&(!y||ytt)&&n==1,Y(ztC,z=gL(z);P(!z,Y(y,yr);0))Y(y&&ytdD||ztdD,Y(y,y=gD(y))z=gD(z))
  P(ztI,I*p=zi,a=!y?m--,*p++:gi(y);int      (*f)(I,I*,L)=arf[0][Av(x)-1][3];mr2(z,a=f(a,p,m));ai(a))
  P(ztL,L*p=zl,a=!y?m--,*p++:gl(y);long long(*f)(L,L*,L)=arf[1][Av(x)-1][3];mr2(z,a=f(a,p,m));al(a))
  P(ztD,D*p=zd,a=!y?m--,*p++:gd(y);double   (*f)(D,D*,L)=arf[2][Av(x)-1][3];mr2(z,a=f(a,p,m));ad(a)))
...
AX(scn,P(xtilIL,enc(x,a,n))P(xtcC,spl(x,a,n))P(Ak(x)<=1,cvi(x,a,n))
 P(n==1,A z=*a;P(ztt||!zn,z)P(zta,z=mut(z);zy=scn(x,&zy,1);z)P(x==cv(','),scn(x,A_(a0(),z),2))
  Y(ari(x),L n=zn-1,f=Av(x);Y(ztC,z=N(gL(z)))
...
AX(eap,P(n==1&&xtilIL,eac(x,a,n))P(n>2,ern(n,a))A z=a[n-1],y=n-1?*a:zt-tS?id_(x,*a):as(0);P(!ztT,et(y,z))P(!zn,yr;z)
 Y(ari(x),L f=Av(x),n=zn;P(yti&&(ztI||ztC) ,I a=gi(gI(y));z=N(gI(z));A u=ax(z);arI(f,n,I b=a;a=zii;uii=)u)
