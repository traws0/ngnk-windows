...
SN A2(enc/*01*/,Q(xtzZ)P(x==ai(2)&&ytz&&gl_(y)<0,x=rshN(64,xR);x(enc(x,y)))
...
SN A2(eac1,/*10*/Yu(eac1f(x,v1[yv]))Xt(y1(x))Xm(eacm(y,&x,1))
...
SN A3(eal2,/*110*/z=prj(z,A(PLH,y),2);z(eac1(x,z)))
SN AX(eal,Ab8;Mc(b,a,8*n);*b=PLH;x=prj(x,b,n);x(eac1(*a,x)))
SN A3(eac2,/*110*/P(ztv,eac2f(x,y,v2[zv]))Xt(Yt(z2(x,y))ear2(x,y,z))Yt(eal2(x,y,z))Xm(eacm(z,A(x,y),2))Ym(eacm(z,A(x,y),2))
...
SN A3(fld2,/*110*/Yt(z2(x,y))i(yN,x=z2(x,get(y,i));B(!x))y(x))
...
SN A2(scn1,/*10*/Xt(x)P(!xN,x)Xm(Az=kv(&x);am(x,Nx(scn1(z,y))))P(y==CAT,scn2(oA,x,y))
...
SN A3(eap2,/*110*/Ym(Au=kv(&y);am(y,Ny(eap2(x,u,z))))Yt(et2(x,y))P(!yN,x(y))P(ztv&&zv<11&&xtzc&&ytZC,arp[zv](x,y,z))
