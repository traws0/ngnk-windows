...
AX(eac,P(n==1,eac1(*a,x))P(n==2,eac2(*a,a[1],x))
 Ab8;Ct[8];Lm=-1;i(n,Ay=b[i]=a[i];Ym(eacm(x,a,n))t[i]=t04(y);I(t[i]>1,L l=yN;P(m>=0&&m-l,el8(a,n))m=l))
...
S A nS(Ax,Lm,OA*a,Nn/*0m1n*/)_(P(n==1,ns(x,m,*a))Az=aA(n+m),*b=zV;zn=n;Mc(b,a,8*n);i(m,mRn(n,b);b[n]=Nz(x8(b,n));zn++;b++)sqz(z))
S A nF(Ax,Lm,OA*a,Nn/*0m1n*/)_(P(n==1,nf(x,m,*a))las(N(nS(x,m,a,n))))
S A wS(Ax,Ay,OA*a,Nn/*0m1n*/)_(P(n==1,ws(x,y,*a))Ab8;Mc(b,a,8*n);mRn(n,b);Az=sqz(aV(tA,n,b));
 W(1,Au=y1(get(z,zn-1));B(!u,z=z(0))B(!tru(u))mRn(n-1,b+1);u=x8(b,n);B(!u,*b=au;z=z(0))Mm(b,b+1,8*n-8);b[n-1]=u;zq(uR))
 mrn(n,b);y(z))
S A wF(Ax,Ay,OA*a,Nn/*011n*/)_(P(n==1,wf(x,y,*a))las(N(wS(x,y,a,n))))
S A3(nws2,/*011*/Yz(ns(x,gl(y),z))YF(ws(x,y,z))et2(y,z))
S A3(nwf2,/*011*/Yz(nf(x,gl(y),z))YF(wf(x,y,z))et2(y,z))
S AX(nws,Ay=*a;P(n==2,nws2(x,y,a[1]))Yz(nS(x,gl(y),a+1,n-1))YF(wS(x,y,a+1,n-1))et8(a,n))
S AX(nwf,Ay=*a;P(n==2,nwf2(x,y,a[1]))Yz(nF(x,gl(y),a+1,n-1))YF(wF(x,y,a+1,n-1))et8(a,n))
S A3(fld2,/*110*/Yt(z2(x,y))i(yN,x=z2(x,get(y,i));B(!x))y(x))
A2(fld1,/*10*/Xt(x)P(!xN,x(id_(y,x)))
 I(ytv&&yv&&yv<11&&xtZDC,XC(fld1(cH(x),y))
  Ct=xt;V(f,arrT[t-tB][yv])P(f,Lv=gl(get(x,0));xe(f(&v,xV+xW,xn-1));t-tD?az(v):ad(*(D*)&v)))
 Az=get(x,0);i(xN-1,z=y2(z,get(x,i+1));B(!z))x(z))
...
S A3(scn2,/*110*/Yt(z2(x,y))Ym(Au=kv(&y);am(y,Ny(scn2(x,u,z))))P(!yN,x(y))
 Au=oA;i(yN,x=z2(x,get(y,i));P(!x,y(u(0)))uq(xR))x(y(u)))
...
S AX(scn,P(n==1,scn1(*a,x))P(n==2,scn2(*a,a[1],x))Ay=*a++;n--;Lm=cfm(a,n);P(m==-2,y(el8(a,n)))
 Az=oA;Ab8;i(m,*b=y;j(n,b[j+1]=get(a[j],i))y=x8(b,n+1);P(!y,mrn(n,a);z(0))zq(yR))mrn(n,a);y(z))
