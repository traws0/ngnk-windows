...
A1 a1,aes0,aes1,asc,ax,blw,cf,cpl,des,dsc,enl,enla,epr,expx,fir,flp,flr,frk,cB,cC,cD,cH,cI,cL,cS,grp,
 hex,js0,js1,kst,las,len,logx,md5,mr,m0,m1,mRa,mut,neg,not,nul,opn,out,prs,rev,sha1,sha256,sha3_256,sinx,ser,spl,sqr,sqz,
...
AL aA,aC,aI,aL,aD,aS,al,az,cls,tilL,rndD;AQ aCl,aCz,cmdcd,cmdl,cmdm,die,e0;
...
V exit(I),iI(V*,N),kinit(),*memchr(Q,I,N),*memcpy(V*,OV*,N),*memmem(OV*,N,OV*,N),*memmove(V*,OV*,N),*memset(V*,I,N),
 mrn(N,OA*),mw(I,N,Q);
C*sl(C*,L),*strchr(Q,I),*strchrnul(Q,I),*strstr(Q,Q),sup(A*,A*),*syp(L*),tZ(L),hx1(I);
I _K(A),asg(A),c3(C,C,C),qD(D,D),eS(A,I),in(L,UL),memcmp(OV*,OV*,N),mtc_(A,A),repl(),si(Q,I),strcmp(Q,Q),syP(Q),rnk(A);
...
#define TZ  0, 8, 1, 2, 4, 8, 8, 1, 4, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8  //item size in bytes
...
#define _W(x) ZT[_t(x)]       //item size in bytes
...
#define VS \
 ...
 au=Lt(tu),FLP,NEG,FIR,SQR,TIL,WHR,REV,ASC,DSC,GRP,NOT,ENL,NUL,LEN,FLR,STR,UNQ,TYP,VAL,U0C,U1C,U2C,U3C,LAS,OUT,
