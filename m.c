...
A1(mr0,asrt(x);P(pkd(x),0)asrt(Ar(x)>0);P(--Ar(x),0)P(!Ab(x),x)mf(x);$(ref(x),mrn(xn,xa))dbg(F(xn*tZ(xt),xci=0xab))0)
A1(mr1,$(x,munmap(xc-ZP,xn+ZP))0)A1(mr,mr1(mr0(x)))V mrn(L n,O A*a)_(F(n,mr(a[i])))A1(mRa,F(xn,mR(xai))x)
A1(mut,P(pkd(x)||Ar(x)==1,x)xr;A y=atnv(xt,xn,xc);$(ref(y),mRa(y))y)
