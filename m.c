...
A1(m0,DBG(lck++);Q(x);XP(0)Q(xr>0);P(--xr,0)Cb=xU;P(!b,x=AT(tn,x))xX=z[b];z[b]=(A)xC;I(xtR,mrn(xn,xA))x)
...
A az(Lv)_(Ct=tZ(v);t==tL?al(v):ai(v))A1(mut,XP(x)P(xr==1,x)x=x(atnv(xt,xn,xC));I(xtR,mRa(x))x)
A room(Ax/*1*/,Nn)_(P(xr==1&&n*ZT[xt]<=xZ,AN(n,x))Ay=atn(xt,n);Mc(yC,xC,xn*ZT[xt]);
 I(ytR,I(xr==1,AN(0,x))E(i(xn,_R(ya))))x(y))
...
A cmdl(Qs)_(Ax=N(u1c(aCl(s)));Ed(!xn||xC[xn-1]-10,x)xC[xn-1]=0;Qp=xC;I(!MQ(p,"#!",2),p=SC0(p+2,10))
...
V kinit(In,O char**a){argv=(Q*)a;env=(Q*)a+n+1;z[ZZ(z)-1]=1;Ax=AN(0,aA(32));glb=aa0();
 ...
 Mc(cn+ti,cn+tI,(tS-tI+1)*SZ(A));i(tn-to,cn[to+i]=au0)
...
A cmdm(Qs)_(obs(xm=0);xys(I(!ytP,_m(y)++));_m(glb)++;_m(cns)++;
...
// hb(hs("mapped regions:"));mms(hb(s=h8(s,(L)p);hs("-");s=h8(s,(L)q);hS(" F",*(C*)p);hS(" N",q-p)))
 ...
 xys(I(!yt,hb(hs("dngl ptr:"));hx(x);hx(y)));au0)
