...
S A ma(UC b)_(asrt(4<b);asrt(b<48);mu+=1ll<<b;A x=mx[b];P(x,mx[b]=nxt(x);AB(b,x))L i=b+1;W(i<ZZ(mx)&&!mx[i],i++)
 $(i<ZZ(mx),x=mx[i];mx[i]=nxt(x))E(i=max(b,24);V*p=mmap_(0,1L<<i,3,0x22/*anon|priv*/,-1,0);P((L)p<=0,err("mem"))mt+=1L<<i;x=(A)(p+ZA))
...
A gkv(A*p)_(A x=*p;asrt(xn==2);P(--Ar(x),*p=mR(xx);mR(xy))mf(x);*p=xx;xy)I sym(A x/*1*/)_(L m=mu-((Ar(x)==1)<<Ab(x));A u=fndpa(&syml,str0(x));mu=m;u)
A atn(UC t,L n)_(UC b=67-__builtin_clzll((ZA>>3)+(t==tC?n>>3:n));A x=N(ma(b));Ar(x)=1;AN(n,AT(t,x)))
A atnv(UC t,L n,O V*v)_(A u=atn(t,n);mc(uc,v,n*tZ(t));u)A1(ax,Ar(x)>1?Ar(x)--,atn(xt,xn):x)A0(aa0,aa(enl(as(0)),enl(cu0)))
...
A1(ldf,x=str0(N(u1c(x)));C*p=xc;W(*p,C*q=p=skp(p);W(*q&&(*q-10||q[1]==32||q[1]=='}'),q++)Nx(line(p,q));p=q+!!*q)xr;cu0)
asm(".globl _start\n_start:pop %rdi\nmov %rsp,%rsi\njmp main");
V main(L n,C**a)_(A x=syml=aX(5);F(xn,xai=aCn(&"_xyzo"[i],!!i))glb=aa0();cn[tX]=aX(0);cn[tC]=cn[tc]=ac(32);
 cn[tL]=cn[tl]=cn[tlx]=al(_0N);cn[tD]=cn[tdx]=ad(_0n);cn[tS]=cn[ts]=as(0);cn[to]=cn[tp]=cn[tq]=cn[tr]=cn[tu]=cn[tv]=cn[tw]=cu0;mu=0;
