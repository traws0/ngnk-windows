...
A1(mr0,dbg(ml++);Q(x);XP(0)Q(Ar(x)>0);P(--Ar(x),0)Cb=Ab(x);P(!b,AT(tn,x))
...
A1(mut,XP(x)P(Ar(x)==1,x)Au=x(atnv(xt,xn,xC));Y(utR,mRa(u))u)size_t strlen(OC*x)_(OC*p=x;W(*p,p++)p-x)
...
A kv(A*p)_(Ax=*p;Q(xn==2);P(Ar(x)>1,--Ar(x);*p=mR(xx);mR(xy))*p=xx;Au=xy;AN(0,x);x(u)) //get keys and values
A room(Ax,Lk)_(Q(xtC);Ln=xn;P(Ar(x)==1&&ZA+n+k<=1ll<<Ab(x),x)Au=AN(n,atn(tC,n+k));mc(uC,xC,n);x(u))
