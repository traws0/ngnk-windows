...
A1(mut,XP(x)P(xr==1,x)Au=x(atnv(xt,xn,xC));Y(utR,mRa(u))u)size_t strlen(OC*x)_(OC*p=x;W(*p,p++)p-x)
V*memcpy(V*x,OV*y,size_t n)_(C*p=x;OC*q=y;i(n,p[i]=q[i])x)V*memmove(V*x,OV*y,size_t n)_(memcpy(x,y,n))
V*memset(V*x,I c,size_t n)_(C*p=x;i(n,*p++=c);x)I strcmp(OC*s,OC*t)_(W(*s&&*s==*t,s++;t++)*s-*t)
I memcmp(OV*x,OV*y,size_t n)_(OC*s=x,*t=y;i(n,Iv=*s++-*t++;P(v,v))0)
A atn(Ct,Ln)_(Ax=ma(64-__builtin_clzll(ZA+7+n*ZT[t]));xr=1;AT(t,AN(n,x)))
A atnv(Ct,Ln,OV*v)_(Au=atn(t,n);mc(uC,v,n*ZT[t]);u)A1(ax,xr>1?xr--,atn(xt,xn):x)A0(aa0,am(enl(as(0)),enl(au0)))
...
V init(){Ax=ce[tS]=aS(0);i(tS,ce[i]=AT(i,x))ce[tm]=am(aS0,aA0);glb=aa0();
 ...
 ci[0][0]=ah(0);ci[1][0]=ai(0);ci[2][0]=al(0);  ci[3][0]=AT(td,ci[2][0]);
 ci[0][1]=ah(1);ci[1][1]=ai(1);ci[2][1]=al(1);  ci[3][1]=ad(1);
 ci[0][2]=      ci[1][2]=      ci[2][2]=al(WL); ci[3][2]=ad(WD);
 ci[0][3]=      ci[1][3]=      ci[2][3]=al(-WL);ci[3][3]=ad(-WD);
 ci[0][4]=      ci[1][4]=      ci[2][4]=cn[tL]; ci[3][4]=cn[tD];}
