...
S A1(mf,asrt(!pkd(x));asrt(!Ar(x));UC b=xb;nxt(x)=mx[b];mx[b]=x;mu-=1ll<<b;0)A1(mut,P(pkd(x)||Ar(x)==1,x)A y=mr2(x,atnv(xt,xn,xc));$(ref(y),mRa(y))y)
V mr0(A x)_(asrt(x);P(pkd(x),)asrt(Ar(x)>0);$(!--Ar(x),mf(x);$(ref(x),mrn(xn,xa)));)
V mr1(A x)_(dbg($(!pkd(x)&&!Ar(x),F((1ll<<xb)-ZA,xci=0xab))))
V mr(A x)_(mr0(x);mr1(x);)
