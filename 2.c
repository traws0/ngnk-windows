...
S A3(ari,Q(ztv)P((1<<xt|1<<yt)&(1<<tm|1<<tM|1<<tA),eac2(x,y,z))N(sup(&x,&y));Ik=xtT<<1|ytT;El(k==3&&xn-yn,x,y)
 ...
 I(t>tD,t+=tD-td)I(z>MXM&&t==tD,t=tI)I(!k,t+=tc-tC;P(_tP(t),Iv=0;f(a,b,&v,1);x(y(az(v)))))
...
A2(mul,adm(x,y,1))
A2(dvd,ari(Ny(cD(x)),Nx(cD(y)),DVD))
A2(mod,Xzd(P(xtl&&*xL==NL,out(x(y)))ari(x,y,MOD))dct(x,y))
A2(mnm,ari(x,y,MNM))
A2(mxm,ari(x,y,MXM))
A2(ltn,ari(x,y,LTN))
A2(gtn,ltn(y,x))
