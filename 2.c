...
S A ari(Ax,Ay,Iv)_(P((1<<xt|1<<yt)&(1<<tm|1<<tM|1<<tA),eac2(x,y,av(v)))N(sup(&x,&y));Ik=xtT<<1|ytT;El(k==3&&xn-yn,x,y)
...
S A ext(Nn,Ax)_(XMT(x)rshL(n,xtm?enl(x):x))
...
A2(mod,Xzd(P(xtl&&*xL==NL,out(x(y)))ari(x,y,vi('!')))dct(x,y))
A2(mnm,ari(x,y,vi('&')))
A2(mxm,ari(x,y,vi('|')))
A2(ltn,ari(x,y,vi('<')))
A2(gtn,ltn(y,x))
A2(eql,xtsS&&ytsS?eql(AT(xt+ti-ts,mut(x)),AT(yt+ti-ts,mut(y))):ari(x,y,vi('=')))
