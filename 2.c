...
OV*arf[][11][4]={{hA(b,B)},{hA(h,H)},{hA(i,I)},{hA(l,L)},{hA(d,D)}};
#undef hA
#undef h
...
ha(B,L,l)ha(H,L,l)ha(I,L,l)ha(L,L,l)ha(D,D,d)
#undef ha
#undef h
#define ha(x) {dex##x,add##x,sub##x,mul##x,dvd##x,mod##x,mnm##x,mxm##x,ltn##x,gtn##x,eql##x},
#define h(x) OV*ar##x[][11]={ha(B##x)ha(H##x)ha(I##x)ha(L##x)ha(D##x)};
h(r)h(s)h(p)h(a)
#undef h
#undef ha

C art(Iv,Ct)_(Q(c3(tc,t,td)||c3(tC,t,tD));Q(in(v,11));Y(t>tD,t+=tD-td)v>7&&t==tD?tI:t) //arithmetic op's result type
I sup(A*p,A*q)_(Ax=*p,y=*q;Ct=max(tH,min(tD,max(tT(xt),tT(yt))));A1*f=CH(t-tB,&gB,gH,gI,gL,gD);*p=x=Ny(f(x));*q=y=Nx(f(y));t)
