...
#define ht0(v,t,T,R)\
 SN I v##T##s(R*RE a,T*RE b,R*RE r,Nn)_(R c=*a;i(PD(n,b),r[i]=c=v##t(c,b[i]))            0)\
 SN I v##T##p(T*RE a,T*RE b,R*RE r,Nn)_(T c=*a;i(PD(n,b),T d=b[i];r[i]=v##t(b[i],c);c=d);0)\
 SN I v##T##a(T*RE a,Nn,L*RE p,L m,T*RE b,I d)_(i(m,Lj=p[i];P(!in(j,n),0)a[j]=v##t(a[j],b[d*i]))0)
#define v0(v,rl,rd) ht0(v,b,B,B)ht0(v,h,H,H)ht0(v,i,I,I)ht0(v,l,L,rl)ht0(v,d,D,rd)
#define ht(v,t,T,Tt,R) ht0(v,t,T,R)\
 SN I v##t##t(T*RE a,T*RE b,R*RE r,Nn)_(*r=v##t(*a,*b);0)\
 SN I v##t##T(T*RE a,T*RE b,R*RE r,Nn)_(AL(b)AL(r)TY(*a)c=*a;i(PD(n,a),*r++=v##t(c,*b++))0)\
 SN I v##T##t(T*RE a,T*RE b,R*RE r,Nn)Tt\
 SN I v##T##T(T*RE a,T*RE b,R*RE r,Nn)_(AL(a)AL(b)AL(r)i(PD(n,a),*r++=v##t(*a++,*b++))0)
