...
S A0(js,s++;A x=aC(0);W(1,C*t=s;C c=*s++;W(c-'"'&&c-'\\'&&c>31,c=*s++)P(c<32,s--;xr;0)x=catc(x,t,s-1-t);P(c=='"',x)c=*s++;
                          I i=si("\"\\/bfnrtu",c);P(i>8,xr;s--;0)x=i<8?apc(x,"\"\\/\b\f\n\r\t"[i]):ju(x);t=s)0)
...
S A0(jv,C c=jw();A x=c=='['?ja():c=='{'?jo():c=='"'?js():c=='-'||dgt(c)?jd():c=='t'?jr("true",1):c=='f'?jr("false",0):c=='n'?jr("null",NL):0;jw();x)
A1(jsr,et(!xtcC,x)x=str0(enla(x));s=xc;A u=jv();Y(u&&*s,ur;u=0)P(!u,eso(x,s-xc);err("json"))xr;u)
A1(jsw,en(1);0)
