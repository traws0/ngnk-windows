...
I mtc_(Ax,Ay/*00*/)_(P(x==y,1)P(xtZ&&ytZ&&xn==yn,gl(rdc(MUL,A(eql(xR,yR)),1)))P(xtz&&ytz,gl_(x)==gl_(y))
 P(xt-yt||xtP||(xtr&&xw-yw)||xn-yn,0)P(!xtR,i(xn*xW,P(xc-yc,0))1)i(xn,P(!mtc_(xa,ya),0))1)
...
S I qA(Ax,Ay/*00*/)_(P(mtc_(x,y),0)P(xtd&&ytd,qD(*xD,*yD))P(xtzc&&ytd,qD(gl_(x),*yD))P(xtd&&ytzc,qD(*xD,gl_(y)))
 ...
 I(!xtP,Iv=qL(xn,yn);XT(i(min(xn,yn),Az=get(x,i),u=get(y,i);I d=qA(z,u);mr(z(u));P(d,d))v))qL(x,y))
...
A1(asc,Xt(opn(x))Xm(K("{(!x)@<. x}",x))XM(K("{(!#x){x@<y x}/|.+x}",x))XS(asc(str(x)))XD(asc(ordD(x)))In=xn;Ez(n-xn,x)
...
A2(bin,Xmt(er2(x,y))YMT(K("{x''y}",x,y))Li=-1,j=xn;W(i+1<j,Lm=(i+j)/2;Az=get(x,m);qA(y,z)<0?j=m:(i=m);z(0))x(y(az(i))))
