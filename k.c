...
AX(prj,XA(app(x,a,n))
 ...
 i(n,k-=a[i]!=PLH)
 Ay=xtp?val(xR):a1(xR);
 Ii=0,j=1;W(i<n&&j<yn,Y(yA[j]==PLH,yA[j]=a[i++])j++)
 W(i<n,yq(a[i++]))
 AT(tp,AK(k,y)))
...
AX(app,XA(idxA(x,a,n))XT(n-1?ern(n,a):idx_(x,*a))Xz(n-1?ern(n,a):wrt(xR,*a))
 ...
 Xr(CH(xw,&eac,rdc,scn,eap,ear,eal)(xx,a,n))
 P(n>k,ern(n,a))Xu(v1[xv](*a))Xw(Ay=*a;AK(0<xv&&xv<4&&na(y)==2?1:ytF?na(y):1,AW(xv,atnv(tr,1,a))))Xo(run(x,a,n))
