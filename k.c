...
AX(app,XtT(xtA?idxA(x,a,n):n-1?ein(n,a):idx_(x,*a))P(xtil,n-1?ern(n,a):wrt(xR,*a))
 ...
 Xtm(Ay=N(idx_(xy,N(fnd(mR(xx),*a))));n-1?dex(y,app(y,a+1,n-1)):y)XtM(Ay=*a;n>1?ern(n,a):ytilIL?idx_(x,y):app(AT(tm,x),a,n))
...
A1(val,XtS(Ay=glb;F(xn,Et(!ytm,x)Az=yx,u=yy;Ed(!ztS||!utA,x)Ik=xi;Ij=fAI(z,k);Ev(j<0,x)y=uA[j])xr;yR)
 XtA(P(!xn,x)P(xn==1,fir(x))Ez(xn>9,x)x=mut(x);Au=app(xx,&xy,xn-1);mr(AN(1,x));u)XtC(x=N(cpl(N(prs(x))));dex(x,run(x,0,0)))
 P(xtc||xts,val(enl(x)))Xtm(las(AT(tA,x)))Xtr(cat(AT(tA,x),aw(xv)))P(c3(tu,xt,tw),al(xv))Et(!(xto||xtp||xtq),x)AT(tA,x))
