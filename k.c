...
S AX(idxX,A y=*a++;n--;
 P(ytX,A b[8];*b=au_plc;mc(b+1,a,n);A z=prj(x,b,n+1);dex(z,eac(z,a-1,1)))
...
AX(app,
 ...
 P(xts,I v=gs(x);A y=*a;n>1?ern(n,a):!v?ser(y):v==Q('j')?json(y):v==Q('k')?kst(y):v==Q('p')?prs(y):v==Q('t')?yr,al(now()):
  v==Q('x')?frk(y):v==Q('e')?yr,epr(),au0:enn(n,a))
 ...
 P(n>xk,ern(n,a))P(xtu,((A1*)vf[xv])(*a))P(xtw,C v=xv;AK(0<v&&v<4&&Ak(*a)==2?1:fun(*a)?Ak(*a):1,AV(v,atnv(tr,1,a))))P(xto,run(x,a,n))
