...
S A valS(I*a,Nn)_(I*k=gk;A*v=gv;Nm=gn;P(!n,mRn(m,v);am(aV(tS,m,k),aV(tA,m,v)))
 W(1,Li=fI(k,m,*a++);P(i<0,ev0())Ax=v[i];P(!--n,xR)P(!xtm,ed0())Ay=xy;x=xx;P(!xtS||!ytA,ed0())k=_V(x);v=_V(y);m=xn)0)
...
S A*pth(Ax/*1*/)_(Xt(pth(enl(x)))
 Li=fI(gk,gn,*xI);I(i<0,gk[i=gn]=*xI;gv[gn++]=xn>1?aa0():au)A*p=gv+i;i(xn-1,P(!stp(i,x,&p),(A*)0))x(0);p)
