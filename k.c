...
S A2(idx_/*x0y1*/,er(xtt,y)P(y==au_plc,xR)P(ytA,ear(cv('@'),A(xR,y),2))P(ytm,y=mut(y);yy=idx_(x,yy);y)et(fun(y),y)P(ytt,getr(x,gl(y)))
 ...
 P(w==3,Lv=gl_(cn[xt]);m2(y,F(un,Lj=yl;ul=in(j,xn)?xL[j]:v)u))
 P(w==2,Iv=gi (cn[xt]);m2(y,F(un,Lj=yl;ui=in(j,xn)?xI[j]:v)u))
 P(w==1,Hv=gh (cn[xt]);m2(y,F(un,Lj=yl;uh=in(j,xn)?xH[j]:v)u))
 Q(!w); Cv=gc (cn[xt]);m2(y,F(un,Lj=yl;uc=in(j,xn)?xC[j]:v)u))
...
S A set(Ax,Li,Ay)_(Q(Ar(x)==1);Y(!xtA&&(!sim(y)||xt-tT(yt)),x=blw(x))
 P(xtA,mr(xa);xa=y;sim(y)?sqz(x):x)xtC?xc=gc(y):xtI?xi=gi(y):xtL?xl=gl(y):xtD?xd=gd(y):xtS?xi=gs(y):0;x)
