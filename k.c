...
X(vc,{':','+','-','*','%','!','&','|','<','>','=','~',',','^','#','_','$','?','@','.','0','1','2','3','4','5'})
X(v1,{sam,flp,neg,fir,sqr,til,whr,rev,asc,dsc,grp,not,enl,nul,len,flr,str,unq,typ,val,u0c,u1c,sam,sam,las,out})
X(v2,{dex,add,sub,mul,dvd,mod,mnm,mxm,ltn,gtn,eql,mtc,cat,exc,rsh,cut,cst,que,ap1,apn,v0c,v1c,dex,dex,com,dex})
X(vn,{___,___,___,___,___,___,___,___,___,___,___,___,___,___,___,___,___,___,amd,dmd,___,___,___,___,___,___})
...
AX(prj,XA(app(x,a,n))Ik=max(n,xK);i(n,k-=a[i]!=PLH)
 Ay=(xtp?val:a1)(xR);Ii=0,j=1;W(i<n&&j<yn,Y(yA[j]==PLH,yA[j]=a[i++])j++)W(i<n,yq(a[i++]))AT(tp,AK(k,y)))
SN AAL(getM,/*0*/Q(xtM);Ay=xy,z=aA(yn);Q(ytA);j(zn,zA[j]=get(yA[j],i))am(_R(xx),sqz(z)))
AAL(get,/*0*/XA(_R(xa))XC(ac(xc))XB(ai(xb))XH(ai(xh))XI(ai(xi))XL(al(xl))XD(ad(xd))XS(as(xi))Xm(getr(xy,i))XM(getM(x,i))xR)
AAL(getr,/*0*/Q(!xtt);in(i,xN)?get(x,i):xN?mkn(get(x,0)):_R(cn[xt]))
...
AX(app,XA(idxA(x,a,n))XT(n-1?ern(n,a):idx_(x,*a))Xz(n-1?ern(n,a):wrt(xR,*a))
 ...
 Ik=xK;P(n<k,prj(x,a,n))Xv(P(n==2,v2[xv](*a,a[1]))vn[xv](a,n))Xr(CH(xw,&w0,w1,w2,eap,ear,eal)(xx,a,n))
 P(n>k,ern(n,a))Xu(v1[xv](*a))Xw(Ay=*a;AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,atnv(tr,1,a))))Xo(run(x,a,n))
 Xp(P(n+xn>8,ern(n,a))I m=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&j<n?a[j++]:_R(xA[i+1]))I l=max(0,n-j);Mc(b+m,a+j,8*l);app(xx,b,m+l))
...
S A set(Ax,Li,Ay)_(Q(xr==1);XA(Az=xa;xa=z(y);ytt&&!ytF?sqz(x):x)P(knd(x)-knd(y)-tC+tc,set(blw(x),i,y))
 Y(xtZ,N(sup(&x,&y)))C w=Zt[xt];w==0?xb=yv:w==1?xh=yv:w==2?xi=yv:(xl=gl(y));x)
...
AA(am1,Ax=*a,y=a[1];Xt(etn(n,a))
 Xm(Az=kv(&x),u=fnd(xR,yR);Y(utlL,Q(ur==1);i(uN,Y(ul==NL,ul=xN;xq(get(y,i));zq(id_(a[2],z)))))
  Ab8;Mc(b+2,a+2,(n-2)*Z*a);*b=y(z);b[1]=u;am(x,Nx(am1(b,n))))
