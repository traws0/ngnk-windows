...
SN A getM(Ax/*0*/,Li)_(Q(xtM);K("{x!y@\\:z}",mR(xx),mR(xy),az(i)))
...
#define iF(f,T,U,R,e...) S L f(OV*RE a,Lm,OV*RE b,V*RE c,UL n)_(O T*p=a;O U*q=b;R*r=c;AL(p)AL(q)AL(r)e;n)
...
#define i_(T,U)iF(i##T##U,T,U,T,UI g=G(T,U);U k[g];\
 ...
 UL l=n/g*g;j(n-l,P(!in(q[j],m),l))j(n-l,r[j]=p[q[j]]))
iC_(I)iC_(L)i_(H,I)j_(H,I)i_(H,L)j_(H,L)i_(I,I)j_(I,I)i_(I,L)j_(I,L)j_(L,I)j_(L,L)iD_(I)iD_(L)iS_(I)iS_(L)
S A2(idx_,/*01*/Er(xtt,y)P(y==PLH,xR)YA(ear(cv('@'),A(xR,y),2))Ym(y=mut(y);yy=idx_(x,yy);y)YF(et1(y))Yt(getr(x,gl(y)))
 Y(!ytL&&!ytI,y=N(gI(y)))Ln=yn;P(xtA||xtM,Au=aA0;Y(ytL,i(n,uq(getr(x,yl))))E(i(n,uq(getr(x,yi))))y(u))
 Au=xtL&&ytL&&yr==1?y:atn(xt,n);Lm=CH(xt-tC+6*ytL,&iCI,iHI,iII,jLI,iDI,iSI,iCL,iHL,iIL,jLL,iDL,iSL)(xC,xn,yC,uC,n);
 Y(m<n,u=u(aL(n));CH(xt-tH+2*ytL,&jHI,jII,jHL,jIL)(xC,xn,yC,uC,n))y-u?y(u):u)
...
AA(am1,Ax=*a,y=a[1],z=a[2];Xt(etn(n,a))
 ...
 Y(n==4,Au=a[3];
  P(xtZ&&ytZ&&ztv&&zv<8&&utzZ,Ei(utT&&yn-un,x(y),u)x=mut(x);y=gL(y);Ny(sup(&x,&u));
   Ei(m2(y,m2(u,x?((int(*)(V*,L,L*,L,V*,I))ara[xt-tH+1][zv])(xC,xn,yL,yn,utP?&u:(V*)uC,utT):0)))x)
