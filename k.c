...
AX(prj,XA(app(x,a,n))Ck=max(n,xk);i(n,k-=a[i]!=au_plc)Au=AK(k,atn(tp,n+1));ux=xR;mc(uA+1,a,8*n);u)
...
AX(app,XT(xtA?idxA(x,a,n):n-1?ein(n,a):idx_(x,*a))P(xtil,n-1?ern(n,a):wrt(xR,*a))
 ...
 Xp(P(n+xn>9,ern(n,a))I m=xn-1,j=0;A b[8];i(m,b[i]=xA[i+1]==au_plc&&j<n?a[j++]:mR(xA[i+1]))I l=max(0,n-j);mc(b+m,a+j,8*l);app(xx,b,m+l))
