...
S A1(rcf,x(ai(xr)))
S A1(jsf,XC(Cb[4096];In=js_eval(xC,xn,b,SZ b);x(0);aCn(b,n))XA(eac1f(x,jsf))et1(x))
A2(app1,/*01*/XA(idxA(x,&y,1))XT(idx_(x,y))Xz(wrt(xR,y))
 Xs(Ss("","k","j","p","cf","t","x","hex","err","epr","argv","env","exit","js","prng","sin","cos","exp","ln","r")
  Sf(x,y,ser,kst,js1,prs,cf,tme,frk,hex,e1f,epr,avf,enf,exf,jsf,prng,ksin,kcos,kexp,klog,rcf))
 XM(YsS(idxm(x,&y,1))idx_(x,y))Xm(idxm(x,&y,1))Ik=xK;P(1<k,prj(x,&y,1))Xv(en1(y))Xr(CH(xw,&w0,w1,w2,w3,w4,w5)(xx,&y,1))
