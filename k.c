...
A2(_1,/*01*/XA(idxA(x,&y,1))XT(idx_(x,y))Xz(dex(v1c(x,yR),y))
 ...
 Xp(P(xn+k>9,er1(y))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&!j?j++,y:_R(xA[i+1]))I l=max(0,1-j);Mc(b+m,&y,8*l);app(xx,b,m+l))
...
AX(app,Q(n);P(n==1,x1(*a))XA(idxA(x,a,n))XM(Ay=*a;YsS(idxm(x,a,n))x=idx_(x,y);P(!x,mrn(n-1,a+1);0)x(app(x,a+1,n-1)))
 ...
 Xq(_1(xx,N(app(xy,a,n))))P(n>k,ern(a,n))Xw(Ay=*a;AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,aV(tr,1,a))))Xo(run(x,a,n))enn(a,n))
A3(_2,/*011*/app(x,A(y,z),2))
