...
AX(app,XA(idxA(x,a,n))XT(n-1?ern(n,a):idx_(x,*a))Xz(n-1?ern(n,a):wrt(xR,*a))
 Xs(P(n>1,ern(n,a))SF(x,*a,"``k`j`p`cf`t`x`hex`md5`sha1`sha256`sha3`aes`sin`err`epr`ptr`argv`env",
                      ser,kst,jsw,prs,cf,tme,frk,hex,md5,sha1,sha256,sha3_256,aes0,sin,err,epr,ptr,argvf,envf))
...
A1(val,XS(Ay=glb;i(xn,Et(!ytm,x)Az=yx,u=yy;Ed(!ztS||!utA,x)Ik=xi;Lj=fAI(z,k);Ev(j<0,x)y=uA[j])x(yR))
 ...
 P(c3(tu,xt,tw),ai(xv))P(xto||xtp||xtq,AT(tA,x))et1(x))
A2(idx,x(idx_(x,y)))
A2(ap1,x(app(x,&y,1)))
