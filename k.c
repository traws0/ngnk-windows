...
A get(Ax/*0*/,Ni)_(XA(mR(xa))XC(ac(xc))XB(ab(xb))XH(ah(xh))XI(ai(xi))XL(al(xl))XD(ad(xd))XS(as(xi))XM(getM(x,i))Xm(get(xy,i))xR)
...
AX(app,XA(idxA(x,a,n))XT(n-1?ern(n,a):idx_(x,*a))Xz(n-1?ern(n,a):wrt(xR,*a))
 Xs(P(n>1,ern(n,a))SF(x,*a,"``k`j`p`cf`t`x`hex`md5`sha1`sha256`sha3`aes`sin`err`epr`ptr`argv`env`exit`ksl`ksr`ksw",
                      ser,kst,jsw,prs,cf,tme,frk,hex,md5,sha1,sha256,sha3_256,aes0,sin,err,epr,ptr,argvf,envf,exitf,ksl,ksr,ksw))
...
A1(val,XS(Ay=glb;i(xn,Et(!ytm,x)Az=yx,u=yy;Ed(!ztS||!utA,x)Ik=xi;Lj=fAI(z,k);Ev(j<0,x)y=uA[j])x(yR))
 XA(P(!xn,x)P(xn==1,fir(x))Ez(xn>9,x)x=mut(x);Au=app(xx,&xy,xn-1);AN(1,x);x(u))
 XC(x=N(cpl(N(prs(x))));x(run(x,0,0)))P(xtc||xts,val(enl(x)))Xm(x(mR(xy)))Xr(cat(AT(tA,mut(x)),aw(xv)))
 P(c3(tu,xt,tw),ai(xv))P(xto||xtp||xtq,AT(tA,mut(x)))et1(x))
A2(idx,x(idx_(x,y)))
A2(ap1,x(app(x,&y,1)))
...
AA(am1,Ax=*a,y=a[1],z=a[2];Xt(etn(n,a))
 ...
 Y(n==4,Au=a[3];
  P(xtZ&&ytZ&&ztv&&zv<8&&utzZ,Ei(utT&&yn-un,x(y),u)x=mut(x);y=gL(y);Ny(sup(&x,&u));
   Ei(m2(y,m2(u,x?((int(*)(V*,N,L*,N,V*,I))ara[xt-tB][zv])(xC,xn,yL,yn,utP?&u:(V*)uC,utT):0)))x)
