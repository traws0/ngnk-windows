...
SN A getM(Ax/*0*/,Nj)_(Q(xtM);Ay=xy,u=aA(yn);Q(ytA);iua(get(ya,j));am(mR(xx),sqz(u)))
A get(Ax/*0*/,Ni)_(XA(mR(xa))XC(ac(xc))XB(ai(xb))XH(ai(xh))XI(ai(xi))XL(al(xl))XD(ad(xd))XS(as(xi))XM(getM(x,i))Xm(get(xy,i))xR)
...
A K0(OC*s,In)_(Lm=mU;Ax=val(aCn(s,n+1));cns=apd(cns,x);mU=m;x)
OC Zt[]={Tz},ZT[]={TZ};
