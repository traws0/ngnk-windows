...
AA(am1,A x=*a,y=a[1],z=a[2];P(n==3&&!fun(z),am1(A_(x,y,av0,z),4))P(xtt,etn(n,a))
 ...
 P(n==4&&(xtIL||xtD)&&ytIL&&ari(z)&&xt==t_lst(At(a[3])),x=mut(x);A u=a[3];L f=Av(z),d=utT;u=enla(u);P(d&&yn-un,el(x,y,u))y=gL(y);
  mr2(y,mr2(u,F(yn,Y(yli<0||xn<=yli,x=ei(x,y,u);B))Y(x,Y(xtD,arD(f,yn,L j=yli;D a=xdj;D b=ud[d*i];xdj=))
                                                      EY(xtI,arI(f,yn,L j=yli;I a=xij;I b=ui[d*i];xij=))
                                                      EY(xtL,arL(f,yn,L j=yli;L a=xlj;L b=ul[d*i];xlj=)))x)))
