...
S A getM(Ax/*0*/,Lj)_(Q(xtM);Ay=xy,u=a0();F(yn,uq(get(ya,j)))am(mR(xx),u))
...
S A2(idx_/*x0y1*/,Er(xtt,y)P(y==au_plc,xR)YA(ear(cv('@'),A(xR,y),2))Ym(y=mut(y);yy=idx_(x,yy);y)Et(fun(y),y)Yt(getr(x,gl(y)))
 y=ytlL?y:N(gI(y));y=N(gL(y));L w=Zt[xt];Au=xtM?a0():atn(xt,yn);P(xtA||xtM,u=AN(0,u);F(yn,uq(getr(x,yl)))yr;u)
