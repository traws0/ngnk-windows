...
S A getM(Ax/*0*/,Lj)_(Q(xtM);Ay=xy,u=a0();i(yn,uq(get(ya,j)))am(mR(xx),u))
...
S A2(idx_/*x0y1*/,Er(xtt,y)P(y==PLH,xR)YA(ear(cv('@'),A(xR,y),2))Ym(y=mut(y);yy=idx_(x,yy);y)Et(fun(y),y)Yt(getr(x,gl(y)))
 y=ytlL?y:N(gI(y));y=N(gL(y));L w=Zt[xt];Au=xtM?a0():atn(xt,yn);P(xtA||xtM,u=AN(0,u);i(yn,uq(getr(x,yl)))yr;u)
 A z=cn[xt];m2(y,CH(w+4*ytlL,&iCI,iHI,iII,iLI,iCL,iHL,iIL,iLL)(xC,xn,yC,uC,un,w==3?*zL:(I)z);0);u)
S AX(idxA,Ay=*a++;n--;YA(A b[8];*b=PLH;mc(b+1,a,n);Az=prj(x,b,n+1);dex(z,eac(z,a-1,1)))
 P(ytT||y==PLH,x=idx_(x,y);P(!n,x)P(!x,mrn(n,a);0)K(".\\:",x,atnv(tA,n,a)))x=idx_(x,y);P(!n,x)P(!x,mrn(n,a);0)Au=idxA(x,a,n);xr;u)
...
A1(val,XS(Ay=glb;i(xn,Et(!ytm,x)Az=yx,u=yy;Ed(!ztS||!utA,x)Ik=xi;Ij=fAI(z,k);Ev(j<0,x)y=uA[j])xr;yR)
...
S A*pth(Ax/*1*/)_(x=enla(x);Ay=glb;Q(xtS&&Ar(y)==1&&ytm&&At(yx)==tS&&At(yy)==tA);Lj=fpI(&yx,*xI);
 Y(j==An(yy),_q(yy,xn>1?aa0():au0))A*p=(A*)dat(yy)+j;i(xn-1,P(!stp(i,x,&p),(A*)0))xr;p)
