...
AX(prj,XA(x8(a,n))Ik=max(n,xK);i(n,k-=a[i]!=PLH)
...
S A2(idx_,/*01*/P(y==PLH||y==au,xR)YA(ear2(xR,y,AP1))Ym(y=mut(y);yy=idx_(x,yy);y)YF(et1(y))Yt(getr(x,gl(y)))
...
S AX(idxm,Ay=N(idx_(xy,N(fnd(_R(xx),*a))));n-1?y(y8(a+1,n-1)):y)
S A1(ktm,x(al(now())))
...
A2(_1,/*01*/XA(idxA(x,&y,1))XT(idx_(x,y))Xz(dex(v1c(x,yR),y))
 Xs(Ss("","k","j","p","cf","t","x","hex","err","epr","argv","env","exit","js","prng","sin","cos","exp","ln","r")
  Sf(x,y,ser,kst,js1,prs,cf,ktm,frk,hex,ker,epr,kav,ken,kex,kjs,prng,ksin,kcos,kexp,klog,krc))
 XM(YsS(idxm(x,&y,1))idx_(x,y))Xm(idxm(x,&y,1))Ik=xK;P(1<k,prj(x,&y,1))Xv(en1(y))Xr(adv[xw](xx,&y,1))
 Xp(P(xn+k>9,er1(y))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&!j?j++,y:_R(xA[i+1]))I l=max(0,1-j);Mc(b+m,&y,8*l);_8(xx,b,m+l))
 Xq(_1(xx,N(_1(xy,y))))P(1>k,er1(y))Xu(v1[xv](y))Xw(AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,aV(tr,1,&y))))
 Xo(run(x,&y,1))Xe(az(((A1*)(x<<8>>8))(gl(N(cL(y))))))et1(y))
A3(_2,/*011*/A a[]={y,z};XA(idxA(x,a,2))XM(YsS(idxm(x,a,2))x=Nz(idx_(x,y));x(x1(z)))Xm(idxm(x,a,2))
 Ik=xK;P(2<k,prj(x,a,2))Xv(v2[xv](y,z))Xr(adv[xw](xx,a,2))
 Xp(P(xn+k>9,er2(y,z))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&j<2?a[j++]:_R(xA[i+1]))I l=max(0,2-j);Mc(b+m,a+j,8*l);_8(xx,b,m+l))
 Xq(_1(xx,N(_2(xy,y,z))))P(2>k,er2(y,z))Xw(AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,aV(tr,1,a))))Xo(run(x,a,2))en2(y,z))
AX(_8,Q(n);P(n==1,x1(*a))P(n==2,x2(*a,a[1]))XA(idxA(x,a,n))XM(Ay=*a;YsS(idxm(x,a,n))x=idx_(x,y);P(!x,mrn(n-1,a+1);0)x(x8(a+1,n-1)))
 Xm(idxm(x,a,n))Ik=xK;P(n<k,prj(x,a,n))Xv(vn[xv](aV(tA,n,a)))Xr(adv[xw](xx,a,n))
 Xp(P(xn+k>9,ern(a,n))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&j<n?a[j++]:_R(xA[i+1]))I l=max(0,n-j);Mc(b+m,a+j,8*l);_8(xx,b,m+l))
 Xq(_1(xx,N(_8(xy,a,n))))P(n>k,ern(a,n))Xw(Ay=*a;AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,aV(tr,1,a))))Xo(run(x,a,n))enn(a,n))
...
A2(idx,x(idx_(x,y)))
A2(ap1,x(x1(y)))
A2(apn,Ym(et2(x,y))Nn=yN;P(!n,y(x))P(n>8,ez2(x,y))y=mRa(Nx(blw(y)));x(y(x8(yA,n))))
...
AA(dmn,Ax=*a,y=a[1];
 P(y==au||!yN,Ab8;*b=y(x);Mc(b+1,a+3,(n-3)*8);Az=a[2];z(z8(b,n-2)))
