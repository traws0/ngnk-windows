...
A2(com,AK(yk,AT(tq,a2(x,y))))AX(prj,P(xtA,app(x,a,n))C k=max(n,xk);F(n,k-=a[i]!=au_plc)Au=AK(k,atn(tp,n+1));ux=xR;mc(ua+1,a,8*n);u)
S A getM(Ax/*0*/,Lj)_(asrt(xtM);Ay=xy,u=a0();F(yn,u=apd(u,get(yai,j)))am(mR(xx),u))
A get(Ax/*0*/,Li)_(xtA?mR(xai):xtI?ai(xii):xtL?al(xli):xtS?as(xii):xtC?ac(xci):xtD?ad(xdi):xtm?get(xy,i):xtM?getM(x,i):xR)
A getr(Ax/*0*/,Li)_(asrt(!xtt);in(i,xn)?get(x,i):xn?mkn(get(x,0)):mR(cn[xt]))
S A2(idx_/*x0y1*/,er(xtt,y)P(y==au_plc,xR)P(ytA,ear(cv('@'),A(xR,y),2))P(ytm,y=mut(y);yy=idx_(x,yy);y)et(fun(y),y)P(ytt,getr(x,gl(y)))
 ...
 P(w==3,  L h=gl_(cn[xt]);m2(y,F(un,Lj=yli;uli=in(j,xn)?xl[j]:h)u))
 P(w==2,  I h=gi (cn[xt]);m2(y,F(un,Lj=yli;uii=in(j,xn)?xi[j]:h)u))
 asrt(!w);C h=gc (cn[xt]);m2(y,F(un,Lj=yli;uci=in(j,xn)?xc[j]:h)u))
...
AX(app,P(xtT,xtA?idxA(x,a,n):n-1?ein(n,a):idx_(x,*a))P(xtil,n-1?ern(n,a):wrt(xR,*a))
 P(xts,P(n>1,ern(n,a))Li=gl(K("``j`k`p`t`x`e`hex`md5`sha1`sha256`sha3`aes`sin?",x));en(i<0,*a)
...
A1(val,P(xtS,Ay=glb;F(xn,et(!ytm,x)Az=yx,u=yy;ed(!ztS||!utA,x)I k=xii;Lj=fndI(z,k);P(j<0,xr;err(syp(k)))y=ua[j])xr;yR)
...
S A*pth(Ax/*1*/)_(x=enla(x);Ay=glb;asrt(xtS&&Ar(y)==1&&ytm&&At(yx)==tS&&At(yy)==tA);Lj=fpI(&yx,*xi);Y(j==An(yy),yy=apd(yy,xn>1?aa0():au0))
 A*p=(A*)dat(yy)+j;F(xn-1,P(!stp(i,x,&p),(A*)0))xr;p)
S A mend(O A*a,In,AA*f)_(A*p=pth(*a);P(!p,mrn(n-1,a+1);0)A b[8];*b=*p;*p=au0;mc(b+1,a+1,(n-1)*8);*p=mR(N(f(b,n))))
...
A K0(O C*s,In)_(L m=mu;Ax=val(aCn(s,n+1));mu=m;x)
O C Zt[]={Tz},ZT[]={TZ};
