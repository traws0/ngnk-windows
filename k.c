...
O L cil[]={_0N,0,0,1,1,_0W,_0W,-_0W,-_0W,_0W,_0N};O D cid[]={_0n,0,0,1,1,_0w,_0w,-_0w,-_0w,_0w,_0n};S A1(__1,en(x))S A2(__2,en(x,y))S A1(sam,x)
...
S A2(idx_/*x0y1*/,asrt(xtT||xtaA);P(y==cv_plc,flp(xR))P(ytX,eac(x,&y,1))P(yta,y=mut(y);yy=idx_(x,yy);y)P(fun(y),et(x,y))P(ytt,getr(x,gl(y)))
 ...
 L h=gl_(cn[xt]);yr;P(w==0,F(un,L j=yli;uci=0<=j&&j<xn?xcj:h)u)P(w==3,F(un,L j=yli;uli=0<=j&&j<xn?xlj:h)u)en(y,u))
AX(app,P(xtT,P(xtX,L c=1;F(n,A y=a[i];L c0=c;c=ytt;x=N(!c0?eal(cV('@'),A_(x,y),2):i?idx(x,y):idx_(x,y)))x)n==1?idx_(x,*a):ein(n,a))
 ...
 P(xta,A y=N(idx_(xy,N(fnd(mR(xx),*a))));P(n==1,y)dex(y,app(y,a+1,n-1)))P(xtA,P(n>1,ern(n,a))A y=*a;P(ytl,idx_(x,y))app(AT(ta,x),a,n))
 P(xtl,P(n>1,ern(n,a))A y=*a;$(ytc,y=enl(y))E(P(!ytC,et(y)))write(gl_(x),yc,yn);y)
...
A1(val,P(xtS,A y=glb;xr;F(xn,P(!yta,et())A z=yx,u=yy;P(!ztS||!utX,ed())L k=xli,j=fndl(z,k);P(j<0,err(C(symstr(k))))y=uaj)yR)
 P(xtX,P(!xn,x)P(xn==1,fir(x))P(xn>8,err("lim",x))x=mut(x);A u=app(xx,&xy,xn-1);mr(AN(1,x));u)
 P(xtC,x=N(cpl(N(prs(x))));dex(x,run(x,0,0)))xtc||xts?val(enl(x)):xta||xtr?las(AT(tX,x)):C3(tu,xt,tw)?al(Av(x)):xto||xtp||xtq?AT(tX,x):et(x))
A2(idx,dex(x,idx_(x,y)))A2(ap1,dex(x,app(x,&y,1)))A2(ap2,y=enla(y);L n=yn;P(!n,yr;x)P(n>8,err("lim",x,y))y=mRa(Nx(blw(y)));A u=app(x,ya,n);xr;yr;u)
S A set(A x,L i,A y)_(asrt(Ar(x)==1);$(!xtX&&(!sim(y)||xt-t_lst(yt)),x=blw(x))
 P(xtX,mr(xai);xai=y;x)P(xtC,xci=gc(y);x)P(xtL,xli=gl(y);x)P(xtD,xdi=gd(y);x)P(xtS,xli=gs(y);x)en(x,y))
...
AA(amd,asrt(n>2);A x=*a,y=a[1],z=a[2];P(xtt,etn(n,a))P(n==3&&!fun(z),amd(A_(x,y,cv0,z),4))
 ...
 P(n==4&&(xtL||xtD)&&ytL&&ari(z)&&xt==t_lst(At(a[3])),x=mut(x);A u=a[3];L f=Av(z),d=utT;u=enla(u);yr;ur;$(d&&yn-un,el(x))
 ...
 rdc(cV('@'),a,n))
S A*pth(A x/*1*/)_(x=enla(x);A y=glb;asrt(xtS&&Ar(y)==1&&yta&&At(yx)==tS&&At(yy)==tX);L j=fndpl(&yx,*xl);$(j==An(yy),yy=apd(yy,xn>1?aa0():cu0))A*p=A(yy)+j;
 F(xn-1,A y=*p;P(!yta,(A*)ed(x))*p=y=mut(y);A z=yx;P(!ztS,(A*)ed(x))L j=fndpl(&yx,xl[i+1]);A u=yy;P(!utX,(A*)ed(x))yy=j==un?apd(u,aa0()):mut(u);
