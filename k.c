...
S A2(idx_,/*01*/Xt(er1(y))P(y==PLH||y==au,xR)YA(ear2(AP1,xR,y))Ym(y=mut(y);yy=idx_(x,yy);y)YF(et1(y))Yt(getr(x,gl(y)))
...
AX(app,XA(idxA(x,a,n))XT(n-1?ern(a,n):idx_(x,*a))Xz(n-1?ern(a,n):wrt(xR,*a))
 ...
 XM(Ay=*a;YsS(idxm(x,a,n))x=idx_(x,y);P(!x,mrn(n-1,a+1);0)n-1?x(app(x,a+1,n-1)):x)Xm(idxm(x,a,n))Ik=xK;P(n<k,prj(x,a,n))
 Xv(n-2?vn[xv](aV(tA,n,a)):v2[xv](*a,a[1]))Xr(CH(xw,&w0,w1,w2,w3,w4,w5)(xx,a,n))
 Xp(P(xn+k>9,ern(a,n))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&j<n?a[j++]:_R(xA[i+1]))I l=max(0,n-j);Mc(b+m,a+j,8*l);app(xx,b,m+l))
 Xq(app(xx,&(A){N(app(xy,a,n))},1))P(n>k,ern(a,n))Xu(v1[xv](*a))Xw(Ay=*a;AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,aV(tr,1,a))))
 Xo(run(x,a,n))Xe(P(n-1,ern(a,n))az(((A1*)(x<<8>>8))(gl(N(cL(*a))))))etn(a,n))
A1(val,XS(Ay=glb;i(xn,P(!ytm,et1(x))Az=yx,u=yy;P(!ztS||!utA,ed1(x))Ik=xi;Lj=fAI(z,k);P(j<0,ev1(x))y=uA[j])x(yR))
 XC(x=str0(x);x(evs(xV)))XA(P(!xn,x)P(xn==1,fir(x))P(xn>9,ez1(x))x=mut(x);Ay=app(xx,&xy,xn-1);AN(1,x);x(y))
 XmM(x(_R(xy)))Xr(cat(AT(tA,mut(x)),aw+xw))P(xtc||xts,val(enl(x)))P(c3(tu,xt,tw),ai(xv))P(c3(to,xt,tq),AT(tA,mut(x)))et1(x))
A2(idx,x(idx_(x,y)))
A2(ap1,x(app(x,&y,1)))
