...
S AX(idxA,Ay=*a++;n--;P(ytT||y==PLH||y==au,x=idx_(x,y);P(!n,x)P(!x,mrn(n,a);0)K(".\\:",x,aV(tA,n,a)))
...
S A1(tme,x(al(now())))
S A1(err,e1(xtC?xV:"err",x))
S A1(argvf,Ay=oA;Q*p=argv;W(*p,yq(aCz(*p++)))ap1(y,x))
S A1(envf,Ay=oS,z=oA;Q*e=env;W(*e,Qp=*e++,q=p+si(p,'=');yq(cS(aCm(p,q)));zq(aCz(q+!!*q)))y=am(y,z);x-au?ap1(y,x):y)
S A1(exitf,exit(xtz?gl(x):1);0)
AX(app,XA(idxA(x,a,n))XT(n-1?ern(n,a):idx_(x,*a))Xz(n-1?ern(n,a):wrt(xR,*a))
 Xs(Ss("","k","j","p","cf","t","x","hex","sin","err","epr","argv","env","exit","e","prng")
    Sf(x,*a,ser,kst,js1,prs,cf,tme,frk,hex,sinx,err,epr,argvf,envf,exitf,expx,prng))
 ...
 Xp(P(n+k>8,ern(n,a))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&j<n?a[j++]:_R(xA[i+1]))I l=max(0,n-j);Mc(b+m,a+j,8*l);app(xx,b,m+l))
 Xq(app(xx,&(A){N(app(xy,a,n))},1))Xe(P(n-1,ern(n,a))az(((A1*)xV)(gl(N(cL(*a))))))etn(n,a))
...
AA(amn,Ax=*a,y=a[1];Xt(etn(n,a))
 ...
 rdc(AP1,a,n))
AA(amd,Ax=*a;Xs(mnd(a,n,amn))amn(a,n))
