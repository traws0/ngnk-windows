...
S A2(idx_,/*01*/Xt(er1(y))P(y==PLH||y==au,xR)YA(ear2(xR,y,AP1))Ym(y=mut(y);yy=idx_(x,yy);y)YF(et1(y))Yt(getr(x,gl(y)))
...
AX(idxm,Ay=N(idx_(xy,N(fnd(_R(xx),*a))));n-1?y(y8(a+1,n-1)):y)
S A1(ktm,x(al(now())))
...
AX(_8,Q(n);P(n==1,x1(*a))XA(idxA(x,a,n))XM(Ay=*a;YsS(idxm(x,a,n))x=idx_(x,y);P(!x,mrn(n-1,a+1);0)x(x8(a+1,n-1)))
 Xm(idxm(x,a,n))Ik=xK;P(n<k,prj(x,a,n))Xv(n-2?vn[xv](aV(tA,n,a)):v2[xv](*a,a[1]))Xr(adv[xw](xx,a,n))
