...
AA(am1,A x=*a,y=a[1],z=a[2];P(n==3&&!fun(z),am1(A_(x,y,av0,z),4))P(xtt,etn(n,a))
 ...
 P(n==4&&(xtIL||xtD)&&ytIL&&ari(z)&&xt==t_lst(At(a[3])),x=mut(x);A u=a[3];L f=Av(z),d=utT;u=enla(u);y=gL(y);yr;ur;$(d&&yn-un,el(x))
  $( xtD,arD(f,yn,L j=yli;$(j<0||xn<=j,xr;x=0)D a=xdj;D b=ud[d*i];xdj=))
  E$(xtI,arI(f,yn,L j=yii;$(j<0||xn<=j,xr;x=0)I a=xij;I b=ui[d*i];xij=))
  E$(xtL,arL(f,yn,L j=yli;$(j<0||xn<=j,xr;x=0)L a=xlj;L b=ul[d*i];xlj=))x)
 P(n==4&&xtC&&ytL&&z==av0&&t_lst(At(a[3]))==tC,x=mut(x);A u=a[3];L d=utT;u=enla(u);yr;ur;$(d&&yn-un,el(x))
  F(yn,L j=yli;$(j<0||xn<=j,xr;x=0)xcj=uc[d*i])x)
 rdc(cv('@'),a,n))
