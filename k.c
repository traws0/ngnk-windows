...
A2(app1,/*01*/XA(idxA(x,&y,1))XT(idx_(x,y))Xz(dex(v1c(x,yR),y))
 ...
 Xq(app1(xx,N(app1(xy,y))))P(1>k,er1(y))Xu(v1[xv](y))Xw(AK(0<xv&&xv<4&&yK==2?1:ytF?yK:1,AW(xv,aV(tr,1,&y))))
 Xo(run(x,&y,1))Xe(az(((A1*)(x<<8>>8))(gl(N(cL(y))))))et1(y))
AX(app,Q(n);P(n==1,app1(x,*a))XA(idxA(x,a,n))XM(Ay=*a;YsS(idxm(x,a,n))x=idx_(x,y);P(!x,mrn(n-1,a+1);0)x(app(x,a+1,n-1)))
