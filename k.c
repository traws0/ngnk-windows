...
AX(prj,XtA(app(x,a,n))Ck=max(n,xk);F(n,k-=a[i]!=au_plc)Au=AK(k,atn(tp,n+1));ux=xR;mc(uA+1,a,8*n);u)
S A getM(Ax/*0*/,Lj)_(Q(xtM);Ay=xy,u=a0();F(yn,u=apd(u,get(ya,j)))am(mR(xx),u))
A get(Ax/*0*/,Li)_(XtL(al(xl))XtI(ai(xi))XtA(mR(xa))XtH(ah(xh))XtS(as(xi))XtC(ac(xc))XtD(ad(xd))Xtm(get(xy,i))XtM(getM(x,i))xR)
...
S A2(idx_/*x0y1*/,Er(xtt,y)P(y==au_plc,xR)YtA(ear(cv('@'),A(xR,y),2))Ytm(y=mut(y);yy=idx_(x,yy);y)Et(fun(y),y)Ytt(getr(x,gl(y)))
...
S AX(idxA,Ay=*a++;n--;YtA(A b[8];*b=au_plc;mc(b+1,a,n);Az=prj(x,b,n+1);dex(z,eac(z,a-1,1)))
 P(ytT||y==au_plc,x=idx_(x,y);P(!n,x)P(!x,mrn(n,a);0)K(".\\:",x,atnv(tA,n,a)))x=idx_(x,y);P(!n,x)P(!x,mrn(n,a);0)Au=idxA(x,a,n);xr;u)
S A2(wrt,Xtl(wrt(gI(x),y))Ytc(wrt(x,enl(y)))Et(!xti||!ytC,y)write(gi(x),yC,yn);y)S A1(fnow,xr;al(now()))
AX(app,XtT(xtA?idxA(x,a,n):n-1?ein(n,a):idx_(x,*a))P(xtil,n-1?ern(n,a):wrt(xR,*a))
 Xts(P(n>1,ern(n,a))SF(x,*a,"``j`k`p`t`x`e`hex`md5`sha1`sha256`sha3`aes`sin",
                       ser,jsw,kst,prs,fnow,frk,epr,hex,md5,sha1,sha256,sha3_256,aes0,sinx))
 Xtm(Ay=N(idx_(xy,N(fnd(mR(xx),*a))));n-1?dex(y,app(y,a+1,n-1)):y)
 XtM(Ay=*a;n>1?ern(n,a):ytilIL?idx_(x,y):app(AT(tm,x),a,n))
 P(n<xk,prj(x,a,n))Xtv(n==2?((A2*)vf[32+xv])(*a,a[1]):x==cv('@')?amd(a,n):x==cv('.')?dmd(a,n):enn(n,a))Xtr(((AX*)vf[58+xv])(xx,a,n))
 P(n>xk,ern(n,a))Xtu(((A1*)vf[xv])(*a))Xtw(Ay=*a;AK(0<xv&&xv<4&&yk==2?1:fun(y)?yk:1,AV(xv,atnv(tr,1,a))))Xto(run(x,a,n))
 Xtp(P(n+xn>9,ern(n,a))I m=xn-1,j=0;A b[8];F(m,b[i]=xA[i+1]==au_plc&&j<n?a[j++]:mR(xA[i+1]))I l=max(0,n-j);mc(b+m,a+j,8*l);app(xx,b,m+l))
 Xtq(app(xx,&(A){N(app(xy,a,n))},1))etn(n,a))
A1(val,XtS(Ay=glb;F(xn,Et(!ytm,x)Az=yx,u=yy;Ed(!ztS||!utA,x)Ik=xi;Ij=fAI(z,k);Ev(j<0,x)y=uA[j])xr;yR)
 XtA(P(!xn,x)P(xn==1,fir(x))Ez(xn>9,x)x=mut(x);Au=app(xx,&xy,xn-1);mr(AN(1,x));u)
 XtC(x=N(cpl(N(prs(x))));dex(x,run(x,0,0)))P(xtc||xts,val(enl(x)))Xtm(las(AT(tA,x)))Xtr(cat(AT(tA,x),aw(xv)))
...
S A set(Ax,Li,Ay)_(Q(Ar(x)==1);Y(!xtA&&(!sim(y)||xt-tT(yt)),x=blw(x))
 XtA(mr(xa);xa=y;sim(y)?sqz(x):x)xtC?xc=gc(y):xtI?xi=gi(y):xtL?xl=gl(y):xtD?xd=gd(y):xtS?xi=gs(y):0;x)
...
AA(am1,Ax=*a,y=a[1],z=a[2];
 Xtm(x=mut(x);xx=unq(cat(xx,yR));xy=cat(xy,rsh(al(An(xx)-An(xy)),id_(z,xy)));
  A b[8];*b=xy;b[1]=fnd(mR(xx),y);mc(b+2,a+2,(n-2)*8);xy=am1(b,n);P(!xy,xy=au0;xr;0)x)
 Xtt(etn(n,a))
 Ytt(x=mut(x);y=gL(y);P(!y,xr;mrn(n-2,a+2);0)
