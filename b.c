...
AX(run,Q(xto);Q(n==xk);S I d;P(++d>1024,esn(a,n))
 UC*b=_V(xy);Im=_n(xA[3]);A*t=s,l[m],r;Mc(l,a,n*SZ x);i(m-n,l[n+i]=au)
 W(1,UC c=*b++;
  I(c>=bc,*--s=_R(_A(xA[4])[c-bc]))
  EI(c>=bm,
   I(c==bm,A*v=l+*b++,x=*v,y=*s++,z=*s;*v=*s=au;*v=dmn(A(x,yR,av+*b++,z),4);B(!*v,r=y(0))*s=apn(_R(*v),y);B(!*s,r=0))
   EI(c==bM,Ax=*s++,y=*s++,z=*s;*s=au;A w=mnd(A(x,yR,av+*b++,z),4,dmn);B(!w,r=y(0))*s=apn(w,y);B(!*s,r=0))
   EI(c==bl,Nn=*b++;s+=n;Ax=sqz(aV(tA,n,s-n));*--s=x)
  ...
  EI(c>=bs,A*v=l+c%16,x=*v;I(c>=bd,*--s=x;*v=au)EI(c>=bg,*--s=xR)E(x=x(*v=_R(*s))))
  EI(c>=bv,Ax=*s++;*s=v2[c-bv](x,*s);B(!*s,r=0))
  EI(c,*s=v1[c](*s);B(!*s,r=0))
  E(B(1,r=*s++)))
 I(!r,eS(_R(xx),_C(xz)[b-1-_C(xy)]);mrn0(s,t-s);s=t)
 mrn0(l,m);d--;r)
