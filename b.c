...
S I lhs(A x/*0*/,A y/*0*/)_(UH o=xo;P(yts&&xx==av0&&Ak(f),L i=fpi(&fl,gs(y));i<16?(lu[i]=An(fb)),h(bs|i),1:err("mxl"))
...
S I rhs(A x/*0*/)_(UH o=xo;Y(xts,P(gs(x)==syC('o'),h(bo);1)L i=fndi(fl,gs(x));P(i>=0,lu[i]=An(fb);h(bg|i);1))P(xtS&&xn==1,hc(as(*xl));1)
 ...
 P(utu&&n==2,h(bu|uv);1)P(utv&&n==3,h(bv|uv);1)N(rhs(xx));h(ba);h(n-1);1)
A1(cpl,UC k=xk;f=x=mut(x);A y=fb;fb=fm=mR(aC(0));fc=a1(au0);C o=0;ms(lu,-1,Z lu);F(yn,P(!rhs(yai),xr;yr;0)h(i==yn-1?br:bp))yr;
...
AX(run,asrt(xto);asrt(n==xk);A f=xR;I m=An(fl);P(sp<stk+m+3,esn(n,a))*--sp=(A)(V*)ip;*--sp=(A)(V*)loc;*--sp=f;ip=dat(fb);loc=sp-1;
