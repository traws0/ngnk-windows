...
enum{bu,bv=32,bs=64,bg=72,bd=80,bi=88,bI,bm,bM,bG,bS,bl,bL,ba,bP,bz,bj,bo,bp,b4,bc};
...
A3(cpl,/*src,ast,loc*/y=cf(y);P(!y,x(z(0)))nb=1;Ik=zn;fl=z;fc=a1(au);Ms(lu,-1,SZ lu);I r=cr(y);y(0);I o=0;h(bu);
...
AX(run,Q(xto)P(n-xk,er8(a,n))S I d;P(++d>1024,es8(a,n))
 ...
 W((c=*b++),
  ...
  J(c>=bi,
   ...
   J(c==bG,A*v=gv+*b++;B(!*v,ev0())p(_R(*v)))
   J(c==bS,A*v=gv+*b++;I(*v,mr(*v))*v=_R(*s);)
   J(c==bl,Nn=*b++;s+=n-1;*s=sqz(aV(tA,n,s-n+1)))
   J(c==bL,Nn=*b++;Ax=*s;B(!xtt&&xN-n,*s=el1(x))i(n,p(ii(x,n-1-i))))
   J(c==ba||c==bP,Nn=*b++;s[n]=(c-ba?prj:_8)(*s,s+1,n);mr(*s);s+=n;B(!*s))
   J(c==bz,b+=1+*b*!tru(q()))
   J(c==bj,b+=*b)
   J(c==bo,p(xR))
   J(c==bp,mr(q()))
