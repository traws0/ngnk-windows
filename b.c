...
S I rhs(A x/*0*/)_(UH o=Ao(x);$(xts,P(gs(x)==4,h(bo);1)L i=fndl(fl,gs(x));P(i>=0,lu[i]=An(fb);h(bg|i);1))P(xtS&&xn==1,hc(as(*xl));1)
 ...
 I p=0;F(n-1,A y=xa[n-1-i];$(y==cv_plc,p=1;hc(yR))E(N(rhs(y))))P(p,N(rhs(xx));h(bP);h(n-1);1)P(u==cv0&&n==2,h(br);1)P(u==cv_mkl,h(bl);h(n-1);1)
...
S A1(vm_/*x0*/,W(1,UC b=*ip++;$(b>=bc,*--sp=mR(A(xa[4])[b&(mxc-1)]))
...
AX(run,asrt(xto);asrt(n==Ak(x));xR;I m=An(xz);P(sp<sp0+m+3,errn("stk",n,a))*--sp=(A)(V*)ip;*--sp=(A)(V*)loc;*--sp=x;ip=C(xy);loc=sp-1;
 F(n,*--sp=a[i])F(m-n,*--sp=cu0)A u=vm_(x);$(!u,eso(mR(xx),C(xa[3])[ip-1-C(xy)]);W(sp<loc-m,$(*sp,mr(*sp))sp++))
