...
S A3(cl,/*0*/Co=yo;P(zts&&yx==av0&&xk,Li=fpI(&fl,gi(z));Ez(i>15)fu[i]=An(fb);h(bs|i);x)
 P(ztS&&!Av(yx),hc(av0);hc(au0);hc(zR);hc(cv('.'));h(ba);h(4);x)
 P(zts||(ztA&&At(zx)==ts),Y(zts,hc(oA))E(i(zn-1,r(x,zA[zn-1-i]))h(bl);h(zn-1))
  Ik=gi(zts?z:zx);Li=fAI(fl,k);Y(i<0,hc(as(k));h(bM))E(fu[i]=An(fb);h(bm);h(i))h(Av(yx));x)
 P(ztA&&zx==MKL,h(bL);h(zn-1);i(zn-1,l(x,y,zA[i+1]);h(bp))1)
 eS(mR(fs),o);ec0())
I asg(Ax)_(xtA||xts||(xtS&&xn>1)) //is lhs of assignment?
S A2(cr,/*0*/Co=yo;Li;P(yts&&(i=fAI(fl,gi(y)))>=0,fu[i]=An(fb);h(bg|i);x)P(yts&&gi(y)=='o',h(bo);x)
