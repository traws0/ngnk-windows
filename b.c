...
S I lhs(A x,A y,A z/*000*/)_(UH o=yo;P(zts&&yx==av0&&xk,L i=fpi(&fl,gs(z));i<16?(lu[i]=An(fb)),h(bs|i),1:err("mxl"))
 ...
 P(zts||(ztX&&At(zx)==ts),Y(zts,hc(a0()))E(F(zn-1,N(rhs(x,za[zn-1-i])))h(bl);h(zn-1))
  I k=gs(zts?z:zx);L i=fndi(fl,k);Y(i<0,hc(as(k));h(bM))E(lu[i]=An(fb);h(bm);h(i))h(Av(yx));1)
 P(ztX&&zx==av_mkl,h(bL);h(zn-1);F(zn-1,N(lhs(x,y,za[i+1]));h(bp))1)en();eso(mR(fs),o);0)
S I rhs(A x,A y/*00*/)_(UH o=yo;Y(yts,P(gs(y)==syC('o'),h(bo);1)L i=fndi(fl,gs(y));P(i>=0,lu[i]=An(fb);h(bg|i);1))P(ytS&&yn==1,hc(as(*yl));1)
 P(ytsS,hc(AO(0,yR));h(bu|vi('.'));1)P(!ytX,hc(y==au_plc?au0:yR);1)L n=yn;P(!n,hc(yR);1)A u=yx;P(utS&&n==1,hc(uR);1)
 ...
 Y(u==cu('*')&&n==2,A z=yy;P(ztX&&zn==2&&zx==cu('|'),N(rhs(x,zy));h(bu|24);1))Y(utw&&n==2,A z=yy;P(!ztX&&!ztsS,zR;hc(app(u,&z,1));1))
