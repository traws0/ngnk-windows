...
A1(cpl,x=AK(xk,AT(to,mut(x)));Ay=fb;fb=oC;fm=oC;fc=a1(au0);xA[5]=aI(16);I o=0;Ms(fu,-1,64);y=cf(y);
 P(!cr(x,y),y(x(dex(xA[5],0))))h(br);y=y(fb);Q(yr==1);j(16,Ii=fu[j];I(i>=0&&yc==bg,yc=bd))dex(xA[5],x))
