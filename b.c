...
#define hc(a) ({I b=bc+fpA(&fc,a);Ez(b>bC)h(b);}) //add a "load constant" instruction
...
S A3(cl,/*0*/UH o=yo;P(zts&&yx==av0&&xk,Li=fpI(&fl,gs(z));Ez(i>15)fu[i]=An(fb);h(bs|i);x)
 ...
 P(ztA&&zx==av_mkl,h(bL);h(zn-1);i(zn-1,l(x,y,zA[i+1]);h(bp))1)eS(mR(fs),o);Ec(1);0)
S A2(cr,/*0*/UH o=yo;Y(yts,P(gs(y)=='o',h(bo);x)Ii=fAI(fl,gs(y));P(i>=0,fu[i]=An(fb);h(bg|i);x))
 ...
 I p=0;i(n-1,Az=yA[n-1-i];z==au_plc?(p=1),hc(zR):r(x,z))
...
AX(run,Q(xto);Q(n==xk);P(s<s0+16,esn(n,a))p((A)c);p((A)l);p(x);c=dat(fb);l=s-1;I m=An(fl);i(n,p(a[i]))i(m-n,p(au0))
