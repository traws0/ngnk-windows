...
A1(cpl,UC k=Ak(x);x=mut(x);A y=xy;xy=aC(0);xa[3]=aC(0);xa[4]=a1(cu0);C o=0;F(yn,x=Ny(rhs(x,yai));h(i==yn-1?br:bp))yr;AK(k,AT(to,x))) //sblmcu
...
S A1(vm_/*x0*/,W(1,UC b=*pc++;$(b>=bc,*--sp=mR(A(xa[4])[b&(mxc-1)]))
 ...
 E$(b>=bd,A*p=loc-(b&(mxl-1));*--sp=*p;dbg(*p=0))E$(b>=bs,A*p=loc-(b&(mxl-1));mr(*p);*p=mR(*sp))E$(b>=bg,A*p=loc-(b&(mxl-1));*--sp=mR(*p))
