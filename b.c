...
A1(cpl,x=AK(xk,AT(to,mut(x)));Ay=fb;fb=oC;fm=oC;fc=a1(au0);xA[5]=aI(16);Ho=0;Ms(fu,-1,64);y=cf(y);Q(*yA==PRG);
 P(!cr(x,y),mr(xA[5]);x(y(0)))y=y(fb);Q(yr==1);j(16,Ii=fu[j];I(i>=0&&yc==bg,yc=bd))mr(xA[5]);x)
...
S A1(vm,/*0*/W(1,Cb=*c++;
 I(b>=bm,I(b>=bc,Ay=fc;p(_R(yA[b-bc])))
        ...
        EI(b==bl,Cn=*c++;s+=n;p(sqz(atnv(tA,n,s-n))))
        EI(b==bL,Cn=*c++;Ax=*s;El(!xtt&&xN-n)i(n,p(get(x,n-1-i))))
        EI(b==ba||b==bP,Cn=*c++;Ax=s[n]=(b==ba?app:prj)(*s,s+1,n);mr(*s);s+=n;N(x))
