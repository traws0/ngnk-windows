...
A2(flt,Er(!ytT,x,y)Au=atn(yt,0);
 i(yn,Az=get(y,i);z=app(x,&z,1);Y(!z,u=u(0);BR)z=gL(z);P(!z,x(y(u(0))))j(gl(z),uq(get(y,i))))x(y(u)))
A2(rsh,XF(flt(x,y))Ym(Xz(Az=kv(&y);xR;y=N2(x,z,rsh(x,y));z=N1(y,rsh(x,z));am(y,z))x=enla(x);xR;am(x,ap1(y,x)))
...
A1(enl,XQ(XP(Au=atn(tT(xt),1);*uL=x;u)AT(tT(xt),x))Xm(Ay=kv(&x);aM(x,ea1(enl,y)))a1(x))
A1(enla,Xmt(enl(x))x)
A2(cat,
