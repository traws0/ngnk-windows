...
A rsh_(A x/*0*/,L m,L*pj,L*s,L r)_(L n=absL(*s);P(r>1,A y=aA(n);F(n,yai=rsh_(x,m,pj,s+1,r-1))y)
...
A2(rsh,P(fun(x),flt(x,y))P(ytm,P(xtl,A z=gkv(&y);xR;y=N2(x,z,rsh(x,y));z=N1(y,rsh(x,z));am(y,z))x=enla(x);xR;am(x,ap1(y,x)))
...
A1(enl,P(sim(x),P(pkd(x),A u=atn(t_lst(xt),1);*ul=x;u)AT(t_lst(xt),x))P(xtm,A y=gkv(&x);aM(x,ea1(enl,y)))a1(x))A1(enla,xtt||xtm?enl(x):x)
A2(cat,P(xtmM&&ytmM,P(xtm&&ytm,A z=gkv(&y);amd(A(x,y,av0,z),4))ed(!mtc_(xx,yx),x,y)A u=eac(cv(','),A(mR(xy),mR(yy)),2);Y(u,u=aM(mR(xx),u))xr;yr;u)
...
A apv(A x,O V*v)_(asrt(xtT);L n=xn,w=ZT[xt];
 Y(Ar(x)==1&&ZA+w+n*w<=1L<<xb,x=AN(n+1,x))E(A y=atn(xt,n+1);Y(Ar(x)>1&&xtA,mRa(x))EY(ref(x),x=AT(tL,x);asrt(ZV==8))m2(x,mc(yc,xc,n*w));x=y)
