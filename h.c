...
A rsh_(Ax/*0*/,L m,L*pj,L*s,L r)_(Ln=absL(*s);P(r>1,Ay=aA(n);F(n,yai=rsh_(x,m,pj,s+1,r-1))y)
 Ct=t_lst(xt);Y(t&&!xn,x=cn[t])Lj=*s>=0?*pj:(m-n%m)%m,w=ZT[t],q=min(m-j,n);*pj=(j+n)%m;Ay=atn(t,n);mc(yc,xc+j*w,q*w);mc(yc+q*w,xc,min(j,n-q)*w);
...
A1(enl,P(sim(x),P(pkd(x),Au=atn(t_lst(xt),1);*ul=x;u)AT(t_lst(xt),x))P(xtm,Ay=gkv(&x);aM(x,ea1(enl,y)))a1(x))A1(enla,xtt||xtm?enl(x):x)
...
A apv(Ax,O V*v)_(asrt(xtT);Ln=xn,w=ZT[xt];
...
A apc(Ax,Cc)_(asrt(xtC);x=room(x,1);xc[xn]=c;AN(xn+1,x))A catc(Ax,O C*s,L l)_(asrt(xtC);x=room(x,l);mc(xc+xn,s,l);AN(xn+l,x))
