...
A rsh_(Ax/*0*/,L m,L*pj,L*s,L r)_(Ln=absL(*s);P(r>1,Ay=aA(n);F(n,ya=rsh_(x,m,pj,s+1,r-1))y)
...
A2(rsh,P(fun(x),flt(x,y))Ytm(Xtl(Az=gkv(&y);xR;y=N2(x,z,rsh(x,y));z=N1(y,rsh(x,z));am(y,z))x=enla(x);xR;am(x,ap1(y,x)))
 YtM(y=mut(y);yy=ear(cv('#'),A(x,yy),2);y)
...
A1(enl,P(sim(x),P(pkd(x),Au=atn(tT(xt),1);*uL=x;u)AT(tT(xt),x))Xtm(Ay=gkv(&x);aM(x,ea1(enl,y)))a1(x))
A1(enla,xtt||xtm?enl(x):x)
A2(cat,P(xtmM&&ytmM,P(xtm&&ytm,Az=gkv(&y);amd(A(x,y,av0,z),4))Ed(!mtc_(xx,yx),x,y)
                    Au=eac(cv(','),A(mR(xy),mR(yy)),2);Y(u,u=aM(mR(xx),u))xr;yr;u)
 x=enla(x);y=enla(y);Y(!xn,SWP(x,y))P(!yn,yr;x)Y(xt-yt,x=Ny(blw(x));y=Nx(blw(y)))L w=ZT[xt],m=xn,n=yn;Au;
 Y(Ar(x)==1&&ZA+(m+n)*w<=1<<Ab(x),u=AN(m+n,x))
 E(u=atn(xt,m+n);mc(uC,xC,m*w);Y(xtA,Y(Ar(x)==1,mr(AN(0,x)))E(mRa(x);xr))E(xr))
...
A2(apd,Q(xtT||xtM);XtM(P(!ytm||!mtc_(xx,yx),apd(Ny(blw(x)),y))x=mut(x);Az=xy=mut(xy);F(An(z),za=apd(za,get(yy,i)))yr;x)
