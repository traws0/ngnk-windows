...
A2(cat,
 ...
 Nm=xn,n=yn;Au=AN(m+n,room(x,m+n));mc(uC+m*ZT[xt],yC,n*ZT[xt]);y(!ytA?0:yr-1?mRa(y):AN(0,y));u)
...
A apv(Ax,OV*v)_(Q(xtT);Nn=xn;x=room(x,n+1);AN(n+1,x);mc(xC+n*ZT[xt],v,ZT[xt]);x)
A apc(Ax,Cc)_(Q(xtC);x=room(x,xn+1);xC[xn]=c;AN(xn+1,x))
A catc(Ax,Qs,Nn)_(Q(xtC);x=room(x,xn+n);mc(xC+xn,s,n);AN(xn+n,x))
