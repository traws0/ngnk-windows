...
A2(rsh,P(fun(x),flt(x,y))Ym(Xl(Az=gkv(&y);xR;y=N2(x,z,rsh(x,y));z=N1(y,rsh(x,z));am(y,z))x=enla(x);xR;am(x,ap1(y,x)))
...
A1(enl,P(sim(x),P(pkd(x),Au=atn(tT(xt),1);*uL=x;u)AT(tT(xt),x))Xm(Ay=gkv(&x);aM(x,ea1(enl,y)))a1(x))
