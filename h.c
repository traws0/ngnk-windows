...
SN A flt(Ax,Ay,Cb)_(En(np(x)-1,x,y)Er(!ytT,x,y)x=Ny(x(app(x,A(yR),1)));x=xN?Ny(gL(x)):x(aL(0));XmMA(et2(x,y))
...
S A drp(Ln,Ax)_(Er(xtt,x)XT(K("{n:#y;y$[n<x|-x;!0;x<0;!n+0^x;x+!n-x]}",az(n),x))
 Xm(Ay=kv(&x);am(Ny(drp(n,x)),Nx(drp(n,y))))en1(x))
A2(cut,Xz(drp(gl(x),y))XF(flt(x,y,0))Xt(en2(x,y))
 Xm(Az=kv(&x);y=fnd(xR,y);P(!y,x(z(0)))yR;am(Nz(cut(x,y)),Nx(cut(z,y))))
 YT(x=Ny(gL(x));K("{y$[|/0<':x,#y;`err\"dom\";x+!'1_-':x,#y]}",x,y))
