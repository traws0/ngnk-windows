...
ALA(rshN,Xm(Ay=kv(&x);x=Ny(rshN(n,x));y=Nx(rshN(n,y));am(x,y))XM(Ay=kv(&x);aM(x,Nx(ear(RSH,az(n),y))))
 ...
 Ay=atn(xt,n);rshV(xC,xn*xW,yC,n*xW);x(ytA?sqz(mRa(y)):y))
S A slc(Ax/*0*/,Li,Lj)_(Q(xtT&&0<=i&&i<=j&&i<=xn)Ay=atn(xt,j-i);Mc(yC,xC+i*xW,(j-i)*xW);XA(sqz(mRa(y)))y)
...
S AAL(rmv,XT(P(!in(i,xn),x)Ay=atn(xt,xn-1);Mc(yC,xC,i*xW);Mc(yC+i*xW,xC+i*xW+xW,(xn-i-1)*xW);I(xtA,y=sqz(mRa(y)))x(y))et1(x))
...
S A insL(Ax,Li,Ln,Ay)_(Xmt(et2(x,y))Ymt(insL(x,i,n,enl(y)))P(xtM||ytM,en2(x,y))
 ...
 Az=atn(xt,xn-n+yn);Mc(zC,xC,i*xW);Mc(zC+i*xW,yC,yn*xW);Mc(zC+(i+yn)*xW,xC+(i+n)*xW,(xn-n-i)*xW);
