...
A2(rsh,XF(flt(x,y,1))Ym(Xz(Az=kv(&y);xR;y=N2(x,z,rsh(x,y));z=N1(y,rsh(x,z));am(y,z))x=enla(x);xR;am(x,ap1(y,x)))
 YM(y=mut(y);yy=ear(cv('#'),A(x,yy),2);y)Yt(rsh(x,enl(y)))P(!yn,rsh(x,enl(fir(y))))
 x=enla(Ny(gL(x)));Nn=xn;P(!n,fir(x(y)))P(n>4,ez2(x,y))
 i(n,Lv=xl;Y(v<0,Ed(n>2||n==2&&v-NL,x,y)P(n==1&&v==NL,x(y))
...
A2(apd,XM(P(!ytm||!mtc_(xx,yx),apd(Ny(blw(x)),y))x=mut(x);Az=xy=mut(xy);i(zn,_q(za,get(yy,i)))y(x))
 Q(xtT);Y(xtZ&&ytz,N(sup(&x,&y)))P(!xtA&&(!ytt||xt-tT(yt)),cat(x,enl(y)))P(!xn,enl(x(y)))Lv=xtA?(L)y:gl(y);apv(x,&v))
...
A apc(Ax,Cc)_(Q(xtC);x=room(x,1);xC[xn]=c;AN(xn+1,x))
A catc(Ax,Qs,Nn)_(Q(xtC);x=room(x,n);mc(xC+xn,s,n);AN(xn+n,x))
