...
fs={},fd=Array(7),env={},E=x=>~console.error(x),N=x=>(alert(x='nyi:'+x),E(x)),log=x=>console.log(x),
...
S('mmap',(p,n,_,_1,f,o)=>{
 ...
 if(f>=0){f=fd[f];if(!f)return E('bad fd');let a=fs[f.p],b=M(p,n);for(let i=o;i<n;i++)b[i]=a[i]}return p})
S('munmap',_=>0)
S('read',(f,a,n)=>{if(f<3){let s=I||prompt('stdin:')+'\n';I='';return te.encodeInto(s,M(a,n)).written}return N('read(f)')})
S('write',(f,a,n)=>f<3?(ap(td.decode(M(a,n))),n):N('write(f)'))
S('gettimeofday',x=>S4(x,[(x=Date.now())/1000|0,x%1000*1000])&0)
S('open',(p,u,_)=>{p=gs(p);let f=3;while(fd[f])f++;if(f>fd.length)return E('max fd')
 if(!fs[p]&&!(u&64/*O_CREAT*/))return E('file not found')
 if(!fs[p]||(u&512/*O_TRUNC*/))fs[p]=new Uint8Array(0)
 fd[f]={p,o:0};return f})
S('close',f=>fd[f]?fd[f]=0:E('bad fd'))
S('lseek',(f,o,w)=>(f=fd[f])?f.o=o+(!w?0:w===1?f.o:fs[f.p].length):E('bad fd'))
