...
F1(sqr,D,__builtin_sqrt(a[i]))F1(sin,D,sind(a[i]))F1(flr,C,a[i]+32*cAZ(a[i]))F1(exp,D,expd(a[i]))F1(log,D,logd(a[i]))
...
R1(flr,XC(fx(flrC,x))Xc(fir(flr(enl(x))))XzZ(x)XsS(K("`$_$:",x))
 x=N(gD(x));Au=atn(xt+tl-td,xn);x2(i(un,ul=xd<0?(L)xd-(xd<(L)xd):(L)xd)u))
R1(sqr,fx(sqrD,N(gD(x))))
R1(sin,fx(sinD,N(gD(x))))
R1(exp,fx(expD,N(gD(x))))
R1(log,fx(logD,N(gD(x))))
