...
A1(neg,XmMA(eac1(x,neg))Xzc(az(-gl(x)))Xd(ad(-gd(x)))XC(neg(cH(x)))
 ...
 et1(x))
A1(not,XmMA(eac1(x,not))XF(x(ai(x==au)))eql(xtsS?as(0):ai(0),x))
A1(nul,XmMA(eac1(x,nul))XF(x(ai(x==au)))eql(_R(cn[xt]),x))

A1(flr,Xc(fir(flr(enl(x))))XC(Az=K("`c$@[!256;65+!26;+;32]");FX(i(PD(xn,xC),yc=zC[xc])))
 XdD(Ay=atn(xt+tl-td,xn);x2(i(yn,yl=xd<0?(L)xd-(xd<(L)xd):(L)xd)y))XsS(cS(flr(str(x))))XzZ(x)XmMA(eac1(x,flr))et1(x))

SN A fx(F1 f,Ax)_(FX(f(xV,yV,xn)))
hf(sqrD,D,i(PD(n,a),r[i]=__builtin_sqrt(a[i]))n)                 A1(sqr ,XmMA(eac1(x,sqr ))fx( sqrD,N(cD(x))))
NLIBC(A1(sinx,en1(x)))LIBC(hf(sinxD,D,i(PD(n,a),r[i]=sin(a[i]))n)A1(sinx,XmMA(eac1(x,sinx))fx(sinxD,N(cD(x)))))
NLIBC(A1(expx,en1(x)))LIBC(hf(expxD,D,i(PD(n,a),r[i]=exp(a[i]))n)A1(expx,XmMA(eac1(x,expx))fx(expxD,N(cD(x)))))
NLIBC(A1(logx,en1(x)))LIBC(hf(logxD,D,i(PD(n,a),r[i]=log(a[i]))n)A1(logx,XmMA(eac1(x,logx))fx(logxD,N(cD(x)))))
