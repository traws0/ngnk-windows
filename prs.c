...
S v1(mon){$(xt==tv&&x!=cv0,R cu(*xl))$(!xt&&xn==2&&xx==cw(0),x=mut10(x);xy=mi(mon(xy)))R x;}
S L pnm(C**p){C*s=*p,c=*s;
 $(c&0x80,DW(c=*++s,(c&0xc0)==0x80))E$(c==':',DW(c=*++s,ldg(c)||c=='.'||c==':'||c=='/'))E(W(ldg(c),c=*++s))
 L v=symidx(*p,s-*p);*p=s;R v;}
S N A pnms(C**p,C sep){C*s=*p,c=*s;A x=as(0);W(1,x=apdl(x,pnm(&s));$(*s!=sep,B)c=*++s)*p=s;R x;}
v1(prs){x=str0(x);C*s=xc,*s0=s;L n=1;A t[64];UC g[ZZ(t)];UH o[ZZ(t)];*t=a2(x,as(0));*g=GG;*o=0;L*na=0;
 W(1,C c=*s;A x=0;C gx=GN;W(c==' ',c=*++s)$(c=='/',DW(c=*++s,c&&c!='\n'));UH ox=s-s0;
  Y(CT(c),ep,
   ...
   Q(CD,$(c=='0'&&s[1]=='x',s+=2;C*p=s;W(CT(*p)==CD||('a'<=*p&&*p<='f'),p++)I m=(p-s)/2;
        ...
        E$(s[1]==':',I u=s[2]==':';s+=2+u;L i=c+(20-'0');gx=GV;x=u?cu(i):cv(i))
        E(L fp=0;C*p=s;c=*p;
          W(1,p+=*p==' ';p+=*p=='-';c=*p;$(!ldg(c),B)
              W(ldg(c)||c=='.'||c==':',fp=fp||c=='.'||c=='n'||c=='w';c=*++p))
          $(fp,x=ad(0);W(1,I m=*s=='-';s+=m;D v=0;c=*s;W(CT(c)==CD,v=10*v+(c-'0');c=*++s)
                           $(c=='.',c=*++s;D w=.1;W(CT(c)==CD,v+=w*(c-'0');c=*++s;w/=10))
                           E$(c=='n',c=*++s,v=_0n)
                           E$(c=='w',c=*++s;v=_0w)
                           x=apdd(x,(1-2*m)*v);$(c!=' '||!num(s+1),B)c=*++s)
               $(xn==1,xt=-xt))
          E(x=al(0);W(1,I m=*s=='-';s+=m;L v=0;c=*s;W(CT(c)==CD,v=10*v+(c-'0');c=*++s)
                        $(c=='W',v=_0W;c=*++s)E$(c=='N',v=_0N;c=*++s)
                        x=apdl(x,(1-2*m)*v);$(c!=' '||!num(s+1),B)c=*++s)
            $(xn==1,xt=-xt))))
   Q(CP,$(s[1]==')',s+=2;x=cea)E(s++;gx=GP;x=a1(cv_lis)))
   Q(CC,psh(GC,a1(as(0)),ox);s0+=ox;C c=*++s;$(c=='[',c=*++s;psh(GB,a1(cv_lis),255);na=0)E(t[n-1]=apda(t[n-1],na=aL(1))))
   Q(CN,L i=n-1;W(g[i]<=GV,i--)$(c!=';'&&c!='\n'&&c!=")]}"[g[i]-GP],ep)
        $(n-i==1,t[n++]=cu0)
        $(g[n-1]==GV,
        ...
        s++;A x=t[i]=apda(t[i],y);
        Y(c,,Q(0,$(n!=1,ep)A y=xa[xn-1];$(!yt&&yn,A z=yx;$(z==cv0||(zt==tu&&yn>2),x=apda(x,cu0)))R x)
             Q(')',g[i]=GN;$(xn==2,md(xx);t[i]=mo(xy);mf_(x)))
             Q(']',g[i]=GN;$(xx==cv_lis,x=cut(cb(1),x);$(xt!=ts,ep)n--;t[n-1]=apda(t[n-1],x));Ao(t[n-1])=o[n-1])
             Q('}',g[i]=GN;xt=tf;md(xx);xx=mi(acm(s0,s));s0-=o[i]-1;
                   S L4 xyz[]={{btr(5,ts,1),1,1},{btr(5,ts,1),2,1,2},{btr(6,ts,1),3,1,2},{3}};
                   y=mo(xy);$(yt==tL,xy=mi(_A(xyz[gL(y)-1])))E(A z=xy=mi(sqz(y));$(zt!=ts,ep))
                   Ao(x)=An(xy);ea(Ao(x));na=0;L j=n-1;W(j>0&&g[j]!=GC,j--)$(j,A z=t[j];z=zy;$(zt==tL,na=zl))))))
 $(x,psh(gx,x,ox))
 $(g[n-1]<=GV,
