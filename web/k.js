...
p1=x=>btoa(c1(pako.deflate(t1(x)))),p0=x=>{try{return t0(pako.inflate(c0(atob(x))))}catch(x){return''}}, //permalinks
...
if(location.hash==='#r'){document.body.classList.add('repl');wa(_=>app.init()) //repl mode
 ...
 rdy(_=>{const h=(location.hash||'')._(1);if(h)ed.value=p0(h)
  ...
  cg.onclick=_=>{const s=ed.value,h='ngn-'+hex8(hash(s))
