...
S I ltr(Cc)_(c3('a',c|32,'z')||c>127)S I ldg(Cc)_(ltr(c)||dgt(c))S I num(C*s)_(dgt(s[*s=='-']))
O C*sc0(O C*s,I c)_(W(*s&&*s-c,s++)s)I si(O C*s,I c)_(sc0(s,c)-s)
S A1(p1,xn-1?x:fir(x))S A pb(A,C);S D p10(In)_(D d=1;F(n,d*=10)d)
...
S A pS(C d)_(Ax=aS(0);W(1,Ay=d-'`'||*s-'"'?ps():pC();y=str0(y);x=apv(x,I(syP(yc)));yr;P(*s-d,x)++s)0)
...
S A pb(Ax,C e)_(W(1,C v=0;Ay=Nx(pe(0,&v));Y(y==au_plc&&(!e||e=='}'),y=au0)x=apv(x,&y);Y(*s-';'&&*s-10,B)s++)ep(*s-e,x)s++;x)
