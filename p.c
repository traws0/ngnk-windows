...
S C*s,*s0,na;S A pb(A,C);S I unh(C c)_((c&15)+9*(c>'9'))S I nu(C*s)_(dg(s[*s=='-']))S A1(p1,xn-1?x:fir(x))I Ci(O C*s,C c)_(O C*t=s;W(*t&&*t-c,t++)t-s)
...
A pS(C**p)_(C*s=*p,c=*s;$(c&0x80,$(cy(c),s+=2;c=*s;W(1,$(dg(c),c=*++s)E$(cy(c),s+=2;c=*s)E(B)))E(W(((c=*++s)&0xc0)==0x80)))
 E$(c==':',W(ld(c=*++s)||Ci("./:",c)<3))E(W(ld(c),c=*++s))A u=aCm(*p,s);*p=s;u)
...
S A pt(C*v)_(A x;C c=*s;P(c=='"',p1(N(pC())))P(c=='`',++s;x=pSs(c);xn>1?enl(x):x)P(c=='(',s++;x=N(pb(enl(cv_mkl),')'));xn-2?x:xy==cv_plc?xr,a0():las(x))
 ...
 I i=Ci("'/\\[",c);P(i<3,c=*++s;I h=c==':';s+=h;*v=1;cw(i+3*h))
 P(nu(s)&&(c-'-'||({C b=s[-1];!ld(b)&&Ci(")]}\"",b)==4;})),P(s[1]==':',L u=s[2]==':';s+=2+u;L i=20+c-'0';*v=1;u?cu(i):cv(i))
  ...
  P(!fp,x=aL(0);W(1,L l=pl(&s);c=*s;$(!l,$(c=='W',l=s[-2]=='-'?-_0W:_0W;c=*++s)E$(c=='N',l=_0N;c=*++s))x=apv(x,&l);$(c-32||!nu(s+1),B)c=*++s)
        $(c=='i',s++;x=N(gI(x)))E$(c=='j',s++)p1(x))
  x=aD(0);W(1,I m=*s=='-';s+=m;D d=0;c=*s;W(dg(c),d=10*d-'0'+c;c=*++s)
              $(c=='.',c=*++s;D w=.1;W(dg(c),d+=w*(c-'0');w/=10;c=*++s))E$(c=='n',c=*++s,d=_0n)E$(c=='w',c=*++s;d=_0w)
              d*=1-2*m;x=apv(x,&d);$(c-32||!nu(s+1),B)c=*++s)p1(x))
 i=Ci(vc,c);P(i>19,cv_plc)I u=*++s==':';s+=u;*v=1;u?cu(i):cv(i))
S A pT(C*v)_(A x=N(pt(v));W(1,C c=*s;I i=Ci("'/\\[",c);P(i>3,x)s++;$(i>2,x=AO(s-1-s0,N(pb(a1(x),']')));$(xn==2&&xy==cv_plc,xy=cu0)*v=0)
                                                                   E(I u=*s==':';s+=u;x=a2(cw(i+3*u),x);*v=1))x)
