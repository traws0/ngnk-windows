...
L absL(Lv)_(v<0?-v:v)C hx1(Ii)_(i+(i>9?'a'-10:'0'))I in(Li,UL n)_(i<n)I c3(Cc,Cv,Cd)_(c<=v&&v<=d)
...
S I num(Qs)_(c09(s[*s=='-']))I si(Qs,Iv)_(SC0(s,v)-s)S D p10(In)_(Dv=1;i(n,v*=10)v)
...
S A pS(Cc)_(Ax=oS;W(1,Ay=c-'`'||*s-'"'?ps():Nx(pC());y=str0(y);y(xq(as(syP(yV))));P(*s-c,x)++s)0)
S A lam(Ik,Ax,Ay,Az)_(AK(k,aV(tA,5,A(x,y,au,z,au)))) //x:src,y:bytecode,z:locals
...
A1(prs,Et(!xtC,x)x=str0(x);Ay=Nx(pk(xV));lam(0,x,y,oS))
