...
S A1(v1,P(xtv&&x-cv0,au(Av(x)))$(xtX&&xn==2&&xx==aw(0),x=mut(x);xy=v1(xy))x)L pu(C**p)_(C*s=*p;L v=0;C c=*s;W(dgt(c),v=10*v+c-'0';c=*++s)*p=s;v)
...
S A pt(C*v)_(A x;C c=*s;P(c=='"',p1(N(pC())))P(c=='`',++s;x=pSs(c);xn>1?enl(x):x)P(c=='(',s++;x=N(pb(enl(cv_mkl),')'));xn-2?x:xy==cv_plc?xr,a0():las(x))
 ...
 i=Ci(vc,c);P(i>19,cv_plc)I u=*++s==':';s+=u;*v=1;u?au(i):av(i))
