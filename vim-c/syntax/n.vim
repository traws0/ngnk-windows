...
sy keyword n_k TD S SN XT NO O ST __attribute__ asm enum do struct for                                          |hi def link n_k statement
...
sy region  n_pp matchgroup=n_p start="\<P(\|\<e[a-z]("    end=")" contains=@n_x,n_p |sy match n_p "," contained |hi def link n_p function
sy region  n_ff matchgroup=n_f start="\<\([WF]\|F[j_]\)(" end=")" contains=@n_x,n_f |sy match n_f "," contained |hi def link n_f special
