...
#define h(a,b,c,d,e,f,g,h) {UI t=h+(rol(e,26)^rol(e,21)^rol(e,7))+ch(e,f,g)+k[r]+w[r],u=(rol(a,30)^rol(a,19)^rol(a,10))+maj(a,b,c);d+=t;h=t+u;r++;}
S V sha256u(UI*v,UL n,UI*s){S O UI k[]={0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,0xd807aa98,0x12835b01,
 ...
 F(n,UI w[64];be4n(w,v,16);F(48,UI p=w[i+1],q=w[i+14];w[i+16]=w[i]+(rol(p,25)^rol(p,14)^p>>3)+w[i+9]+(rol(q,15)^rol(q,13)^q>>10))
  UI a=*s,b=s[1],c=s[2],d=s[3],e=s[4],f=s[5],g=s[6],h=s[7],r=0;
