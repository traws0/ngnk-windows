...
S A md(Ax/*1*/,OV*iv,N nv,void(*f)(O U*,N,U*),I b)_(Et(!xtC,x) //merkle-damgard construction with padding
 I a[nv];UC c[128]={};Mc(a,iv,SZ a);Nn=xn,k=n/64,r=n%64,m=n+72&~63;x2(f(xV,k,a);Mc(c,xV+n-r,r));c[r]=128;
...
S V ks(UL*a){S O UL z=1<<15,y=1ll<<31,x=1ll<<63,t[]={1,z+130,x+z+138,x+y+z,z+139,y+1,x+y+z+129, //keccak sponge
 ...
 i(24,UL s[7],b[25];Ms(s,0,SZ s);
  ...
  i(25,b[i]=R(a[i],UC(0,1,62,28,27,36,44,6,55,20,3,10,43,25,39,41,45,15,21,8,18,2,61,56,14)[i])) //rho
  i(25,a[i]=b[UC(0,6,12,18,24,3,9,10,16,22,1,7,13,19,20,4,5,11,17,23,2,8,14,15,21)[i]])          //pi
  i(5,UL*r=a+5*i,x=*r,y=r[1];F5(UL z=r[4-i];r[4-i]^=~x&y;y=x;x=z))                               //chi
  *a^=t[i])}                                                                                     //iota
S V kec(U r,O UC*p,Nn,UC s,UC*z,U d){U b=0,i;UC a[200];Ms(a,0,SZ a);
...
S UC s0[256],s1[256];S UC t0(UC c)_(c<<1^27*(c>>7))S UC t1(UC c,UC d)_(UC v=c*(d&1);i(4,c=t0(c);v^=(d>>i+1&1)*c)v)
S V aei(){UC c=1,d=1;Ms(s0,99,256);i(255,c^=t0(c);i(3,d^=d<<(1<<i))d^=(d>>7)*9;i(5,s0[c]^=R(d,i)))i(256,s1[s0[i]]=i)}
S V x16(UC*r,O UC*p){i(16,r[i]^=p[i])}S V i16(UC*r,O UC*p,O UC*q){i(16,r[i]=p[q[i]])}
S V kxp(V*w0,OV*k,U l){U*w=w0,t;Mc(w,k,4*l);UC*p=(V*)&t;
 i(3*l+28,t=w[i+l-1];I(i%l==0,t=R(t,24);j(4,p[j]=s0[p[j]])t^=UC(141,1,2,4,8,16,32,64,128,27,54)[i/l+1])
                     EI(l==8&&i%l==4,i(4,p[i]=s0[p[i]]))w[i+l]=w[i]^t)}
S V mx0(UC*a){i(4,UC t[4];i(4,t[i]=a[i]^a[i+1&3])UC c=*t^t[2];i(4,a[i]^=t0(t[i])^c)a+=4)}
S V mx1(UC*a){i(4,UC t[4]={};i(4,j(4,t[i]^=t1(a[j],UC(14,11,13,9)[j-i&3])))Mc(a,t,4);a+=4)}
S V ae0(V*a,OV*w,U r){UC b[16];
 i(r,x16(a,w+16*i);i16(b,s0,a);i16(a,b,UC(0,5,10,15,4,9,14,3,8,13,2,7,12,1,6,11));i<r-1?mx0(a):x16(a,w+16*r))}
S V ae1(V*a,OV*w,U r){UC b[16];x16(a,w+16*r);
 i(r,i16(b,a,UC(0,13,10,7,4,1,14,11,8,5,2,15,12,9,6,3));Mc(a,b,16);i16(a,s1,a);x16(a,w+16*(r-1-i));I(i<r-1,mx1(a)))}
S V ecb(V*a,OV*k,U l,U d){UC w[l+7<<4];kxp(w,k,l);(d?ae1:ae0)(a,w,l+6);}
