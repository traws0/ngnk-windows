...
S U bl(U x)_(x<<24|x<<8&0xff0000|x>>8&0xff00|x>>24)S UL bL(UL x)_(bl(x>>32)|(UL)bl(x)<<32)S U*bn(U*r,O U*a,U n)_(i(n,r[i]=bl(a[i]))r) //big-endian
...
S V sha256u(U*v,UL n,U*s){S O U k[]={0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,0xd807aa98,0x12835b01,
 ...
 i(n,U w[64],t[8],r=0;bn(w,v,16);i(48,U p=w[i+1],q=w[i+14];w[i+16]=w[i]+(rot(p,25)^rot(p,14)^p>>3)+w[i+9]+(rot(q,15)^rot(q,13)^q>>10))
     F8(t[i]=s[i])i(8,h(0)h(1)h(2)h(3)h(4)h(5)h(6)h(7))F8(s[i]+=t[i])v+=16)}
...
S V ks(UL*a){S O UL z=1<<15,y=1ll<<31,x=1ll<<63,t[]={1,z+130,x+z+138,x+y+z,z+139,y+1,x+y+z+129,x+z+9,138,136,y+z+9,y+10,y+z+139, //keccak sponge
 ...
 i(24,UL s[7],b[25];ms(s,0,Z s);
  i(5,F5(s[1+i]^=*a++))*s=s[5];s[6]=s[1];F5(s[i]^=rot(s[i+2],1))a-=25;i(5,F5(*a++^=s[i]))a-=25;   //theta
  i(25,b[i]=rot(a[i],C(0,1,62,28,27,36,44,6,55,20,3,10,43,25,39,41,45,15,21,8,18,2,61,56,14)[i])) //rho
...
S V aei(){Cc=1,d=1;ms(s0,99,256);i(255,c^=t0(c);i(3,d^=d<<(1<<i))d^=(d>>7)*9;i(5,s0[c]^=rot(d,i)))i(256,s1[s0[i]]=i)}
S V x16(C*r,OC*x){i(16,r[i]^=x[i])}S V i16(C*r,OC*x,OC*y){i(16,r[i]=x[y[i]])}
S V kxp(V*w0,OV*k,U l){U*w=w0,t;mc(w,k,4*l);C*p=(V*)&t;
 i(3*l+28,t=w[i+l-1];Y(i%l==0,t=rot(t,24);j(4,p[j]=s0[p[j]])t^=C(141,1,2,4,8,16,32,64,128,27,54)[i/l+1])EY(l==8&&i%l==4,i(4,p[i]=s0[p[i]]))w[i+l]=w[i]^t)}
