...
A1(u1c,I f=N(hop_(x,O_RDONLY));struct stat s;P(fstat(f,&s),err("stat"))L n=s.st_size;
 V*p=mmap_(0,n+ZP,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANON,-1,0);P(((L)p>>4)==-1,err("mmap"))
 A u=(A)(p+ZP);asrt(!(u%ZP));ul[-2]=0;u=AT(tC,AN(n,u));uR;
 V*q=mmap_(uc,n,PROT_READ,MAP_PRIVATE|MAP_FIXED,f,0);close(f);P(((L)q>>4)==-1,err("mmapfile"))asrt(q==uc);u)
