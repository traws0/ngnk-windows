...
S L hop_(A x/*1*/,L fl)_(P(xtil,hskt(lh,gi(x)))P(xts,x=symstr(gs(x));!*xc?1:*xc-':'?et(),0:hops(xc+1,fl))P(xtC,x=str0(x);dex(x,hops(xc,fl)))et(x))
A1(hop,ai(N(hop_(x,O_RDWR|O_CREAT))))A1(hcl,asrt(xti);close(gi(x));cu0)
